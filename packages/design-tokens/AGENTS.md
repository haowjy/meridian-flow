...
consuming them in apps. The file header owns the default palette story;
`src/themes.css` owns the shipped dark-theme role overrides. Do not duplicate
either in `.context/`.
...
- A value shared by several role tokens is defined once as a package-private
  ...
  components use role tokens only. Dark mode overrides semantic roles and
  the private lift/recess atoms in `src/themes.css`; consumers still use roles.
