/**
 * Library screen contract — the workbench's full capability inventory.
 *
 * Distinct from the catalog (`WorkbenchAgentsResponse`): the catalog lists only
 * agents *selectable for new threads* (enabled + primary); the Library lists
 * everything the workbench owns — including disabled agents and skills not
...
/** Response shape for `GET /api/workbenches/:workbenchId/library`. */
export interface WorkbenchLibraryResponse {
