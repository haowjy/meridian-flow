 ...
 * the workbench inventory surface as `keep_existing` (skip-and-keep), matching
...
/** Request body for `POST /api/workbenches/:wb/packages/preview`. */
...
/** Slug already owned by the workbench — apply keeps the existing definition. */
...
export interface PackageInstallPreviewResponse {
  ...
  /** Package names already installed in this workbench (dependency graph skips). */
  skippedPackages: string[];
}

/** Request body for `POST /api/workbenches/:wb/packages` (apply install). */
