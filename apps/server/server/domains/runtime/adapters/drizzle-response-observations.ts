    ...
    async freezeCausalCuts(documentIds) {
      ...
      const rows = await currentDrizzleDb(db)
        .select({
          ...
          admittedThrough: documentYjsHeads.latestUpdateSeq,
        })
        .from(documentYjsHeads)
        .where(inArray(documentYjsHeads.documentId, uniqueIds as DocumentId[]));
      const byDocument = new Map(rows.map((row) => [row.documentId, row.admittedThrough]));
      return uniqueIds.map((documentId) => ({
        id: crypto.randomUUID(),
        version: 1,
        documentId: documentId as DocumentId,
        // J1 replaces this initial-generation identity with branch-local authority generation.
        authorityId: documentId as DocumentAuthorityId,
        generation: INITIAL_DOCUMENT_AUTHORITY_GENERATION,
        admittedThrough: BigInt(byDocument.get(documentId) ?? 0),
      }));
    },
  };
}
