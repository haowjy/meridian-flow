...
function seedPackageWithoutInvoke(projectId: string, skills: SkillRecord[]) {
...
function seedPackage(projectId: string, skills: SkillRecord[], agentBody = "Agent body.") {
  const agent: AgentDefinitionRecord = {
    id: "agent-1",
    projectId,
    slug: "agent-one",
...
function seedTwoAgentPackage(projectId: string) {
...
async function setupOrchestrator(
  buildPackage: (projectId: string) => ReturnType<typeof createInMemoryPackageStore>,
) {
  const projectRepo = createInMemoryProjectRepository();
  const repos = createInMemoryRepositories({ projects: projectRepo });
  const project = await projectRepo.create({ userId: "user-1", title: "WB" });
  const packageRepository = buildPackage(project.id);
  ...
  return { repos, orchestrator, project, ...capture, creditLedger };
}
...
describe("composed system prompt freeze", () => {
  it("keeps turn 2 system prompt byte-identical after a skill is installed between turns", async () => {
    ...
    const { repos, orchestrator, project, systemTexts, toolNames, creditLedger } =
    ...
    const packageAfterInstall = seedPackage(projectId, [
      initialSkill,
      skillRecord(projectId, "skill-two"),
    ]);
  ...
  it("does not overwrite a pre-frozen subagent composedSystemPrompt on first attempt", async () => {
    ...
    const { repos, orchestrator, project, systemTexts } = await setupOrchestrator((id) => {
  ...
  it("persists composedSystemPrompt only on the first turn", async () => {
    const { repos, orchestrator, project } = await setupOrchestrator((id) =>
  ...
  it("does not advertise invoke when the baked skill set is empty", async () => {
    const { repos, orchestrator, project, toolNames } = await setupOrchestrator((id) =>
  ...
  it("re-bakes legacy raw body when systemPrompt is set but bakedSkillSlugs is null", async () => {
    let packageRepository!: ReturnType<typeof createInMemoryPackageStore>;
    const { repos, project } = await setupOrchestrator((id) => {
  ...
  it.each([
    ...
    const repos = createInMemoryRepositories({ projects: projectRepo });
    const project = await projectRepo.create({ userId: "user-1", title: "WB" });
    const nonInvocableSkill = skillRecord(project.id, "skill-non-invocable");
    const invocableSkill = skillRecord(project.id, "skill-invocable");
    const zeroSkillPackage = seedPackageWithoutInvoke(project.id, [nonInvocableSkill]);
    const oneSkillPackage = seedPackage(project.id, [invocableSkill], "Agent body.");
