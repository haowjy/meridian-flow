...
describe("runtime credits", () => {
  it("lets an in-flight turn complete after its debit drains the balance, then blocks the next turn", async () => {
    ...
    expect(
      await creditLedger.getBalance({ userId: "user-1", workbenchId: thread.workbenchId }),
    ).toBe("0");

    await expect(
      orchestrator.runTurn({ threadId: thread.id, userText: "second" }),
