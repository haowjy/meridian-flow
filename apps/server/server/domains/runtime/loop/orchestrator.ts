...
export interface OrchestratorDeps {
  ...
  workbenchPreferences: {
    read(userId: string, workbenchId: string): Promise<WorkbenchPreferences>;
  };
  permissionGate: PermissionGate;
  /** Workbench-scoped ledger for model-call credit balance checks, debits, and spawn rollups. */
...
async function resolveCheckpointAutoResumePolicy(
  ...
  const preferences = await deps.workbenchPreferences.read(thread.userId, thread.workbenchId);
...
export async function runTurn(deps: OrchestratorDeps, input: RunTurnInput): Promise<RunTurnHandle> {
  ...
  if (balance <= 0n) {
    throw meridianErrorFromSystem(
      "credits_exhausted",
      "Workbench credits are exhausted; add credits before starting a new turn",
    );
  }

  const priorTurns = await repos.turns.listByThread(input.threadId);
  const lastTurn = priorTurns.at(-1) ?? null;
...
async function* generateEvents(
  ...
  const allTurns: Turn[] = [...priorTurns, userTurn, assistantTurn];
  const allBlocks: Block[] = await repos.blocks.listByThread(input.threadId);
