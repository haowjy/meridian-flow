...
| `reference-context.ts` | Before the first model call, loads the current user turn's authorized text references through the host-wired agent-edit read operation. Reads run outside admission/persistence transactions; results are saved on reference blocks before gateway submission. Duplicate identities read once per turn; history replays saved results. Image references retain their separate projection. Client admission rejects read payloads. |
