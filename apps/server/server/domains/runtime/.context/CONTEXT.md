...
| `ports.ts` | Notification/steering vocabulary and ports: `Inbox` (durable per-thread queue with idempotency and at-least-once claim/ack), `RunAuthority` (session advisory lock as the mutex plus an expiring, queryable lease row), and the thin `RunStarter` seam. Drizzle adapters: `adapters/drizzle-inbox.ts` and `adapters/drizzle-thread-run-ownership.ts` (`createDrizzleRunAuthority`); in-memory fakes under `adapters/in-memory/loop-ports.ts`. Backed by `thread_inbox_messages` and `thread_run_leases`. |
