...
| Prompt cache breakpoints | `loop/prompt-cache-marks.ts` sets ephemeral `providerOptions.anthropic.cacheControl` on the frozen system message's last content part (covers tools + system, since tools precede system in Anthropic wire order) and on the final message's last content part (extends the cache incrementally over history), applied in `turn-context-assembly.ts` right after `buildContext()` and gated on the resolved model's `"caching"` capability from the registry — config-driven, not a provider-name check. Two breakpoints, well under Anthropic's 4-mark limit. The Anthropic adapter already forwarded `providerOptions.anthropic.cacheControl` for text/tool_use/tool_result/function-tool blocks; the OpenAI-compatible mapper (shared by OpenRouter) additionally forwards it as an OpenRouter-style `cache_control` array block for Anthropic-backed models. OpenAI/DeepSeek models have no `"caching"` capability today and get no marks — their own automatic prefix caching needs nothing beyond the stable frozen prefix this same change delivers; no `prompt_cache_key` is wired since `GenerateRequest` is deliberately thread-agnostic. |
