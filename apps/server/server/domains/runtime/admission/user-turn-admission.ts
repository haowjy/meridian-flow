...
/**
 * Turns an expired `pending` record into `rejected` only after holding the run
 * claim so a live writer/runner cannot be reclaimed under it.
 */
...
export function createUserTurnAdmission(deps: {
  ...
  const recover = (
    ...
    submissionId: string,
    record: AdmissionRecord | null,
  ): Promise<AdmissionRecord | null> => recoverAdmission(deps, threadId, submissionId, record);
  return {
