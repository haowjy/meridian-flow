...
import type { ThreadId, TurnId, UserId, WorkbenchId, WorkId } from "@meridian/contracts/runtime";
import type {
  Block,
  BlockStatus,
  BlockType,
...
export interface ThreadRepository {
  ...
  /** Threads in a workbench (excludes soft-deleted threads; caller must gate workbench access). */
  listByWorkbench(workbenchId: WorkbenchId): Promise<ThreadListItem[]>;
  ...
  //   - exclude status:"archived" from listByWorkbench / listByWork by default, and
...
export type InternalThreadRepositories = ThreadRepositories & {
  threads: ThreadRepository & SubagentThreadFactory;
};
