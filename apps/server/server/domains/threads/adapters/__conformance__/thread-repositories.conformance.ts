...
import type { WorkRepository } from "../../../workbenches/ports/work-repository.js";
...
export function describeThreadRepositoriesConformance(
  ...
  describe(`ThreadRepositories conformance: ${name}`, () => {
    it("creates a root thread and rejects spawn-shaped input", async () => {
      const { repos, workbenches } = await makeFixture();
    ...
    it("finds threads by id and lists by user and workbench", async () => {
      const { repos, workbenches } = await makeFixture();
      const workbench = await workbenches.create({
        userId: THREAD_REPOSITORIES_CONFORMANCE_USER_ID,
        title: "Workbench",
      ...
      await expect(repos.threads.listByWorkbench(workbench.id)).resolves.toHaveLength(1);
    });

    it("projects work title, waiting state, running turn, and listByWork", async () => {
      const { repos, workbenches, works } = await makeFixture();
      const workbench = await workbenches.create({
        userId: THREAD_REPOSITORIES_CONFORMANCE_USER_ID,
        title: "Workbench",
      });
      const otherWorkbench = await workbenches.create({
        userId: THREAD_REPOSITORIES_CONFORMANCE_USER_ID,
        title: "Other Workbench",
      });
      ...
      const crossWorkbenchWork = await works.create({
        workbenchId: otherWorkbench.id,
        title: "Cross Workbench",
      });
      ...
      await repos.threads.create({
        userId: THREAD_REPOSITORIES_CONFORMANCE_USER_ID,
        workbenchId: otherWorkbench.id,
        workId: crossWorkbenchWork.id,
        title: "Cross Workbench Thread",
      });

      const projectThreads = await repos.threads.listByWorkbench(workbench.id);
      ...
      const workThreads = await repos.threads.listByWork(workbench.id, work.id);
      expect(workThreads.map((thread) => thread.id).sort()).toEqual(
        [waitingThread.id, runningThread.id].sort(),
      );
      await expect(repos.threads.listByWork(otherWorkbench.id, work.id)).resolves.toEqual([]);
    ...
    it("soft-deletes and restores threads idempotently", async () => {
      ...
      await expect(repos.threads.listByWorkbench(workbench.id)).resolves.toEqual([]);
    ...
    it("hides threads when the workbench is soft-deleted", async () => {
      const { repos, workbenches } = await makeFixture();
      const workbench = await workbenches.create({
        userId: THREAD_REPOSITORIES_CONFORMANCE_USER_ID,
        title: "Workbench",
      ...
      await workbenches.softDelete(workbench.id);
      expect(await repos.threads.findById(thread.id)).toBeNull();
      await expect(repos.threads.listByWorkbench(workbench.id)).resolves.toEqual([]);

      await workbenches.restore(workbench.id);
      expect(await repos.threads.findById(thread.id)).toMatchObject({ id: thread.id });
    });
