...
if (!RUN_DB_TESTS || !DATABASE_URL) {
  ...
  describe("drizzle thread repositories (postgres)", async () => {
    ...
    const { createDrizzleWorkbenchRepository } = await import(
      "../../../workbenches/adapters/workbench-repository/drizzle.js"
    );
    const { createDrizzleWorkRepository } = await import(
      "../../../workbenches/adapters/work-repository/drizzle.js"
    );
    ...
    describeThreadRepositoriesConformance("drizzle", () => {
      const workbenches = createDrizzleWorkbenchRepository({ db });
      const works = createDrizzleWorkRepository({ db });
      const repos = createDrizzleRepositories(db);
      return { repos, workbenches, works };
    });
    ...
    describeThreadDocumentRepositoriesConformance("drizzle", () => {
      ...
      return {
        repos,
        workbenches,
        async createDocument(workbenchId: string, filename: string) {
