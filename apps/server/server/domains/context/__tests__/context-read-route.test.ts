...
function contextFactoryFor(port: ContextPort): ContextPortFactory {
  return {
    forWorkbench: (_workbenchId: string, _userId: string) => port,
    forThread() {
...
describe("context read route core", () => {
  ...
  it("returns tracked content with schema and language metadata", async () => {
    ...
    const response = await handleContextReadRequest(
      ...
      { workbenchId: "workbench-1", userId: "user-1", scheme: "kb", rawPath: "/notes/analysis.py" },
    );
  ...
  it("returns tracked extensionless content with markdown defaults", async () => {
    ...
    const response = await handleContextReadRequest(
      ...
      { workbenchId: "workbench-1", userId: "user-1", scheme: "kb", rawPath: "/wefwef" },
    );
  ...
  it("returns tracked unknown-extension content from stored markdown classification", async () => {
    ...
    const response = await handleContextReadRequest(
      ...
      { workbenchId: "workbench-1", userId: "user-1", scheme: "kb", rawPath: "/notes.wef" },
    );
  ...
  it("returns a signed URL for non-tracked storage-backed files", async () => {
    ...
    const put = await objectStore.put(
      "context/workbench-1/data/report.pdf",
      Buffer.from("%PDF"),
      "application/pdf",
    );
    ...
    expect(response).toEqual({
      ...
      url: "/memory-object-store/context%2Fworkbench-1%2Fdata%2Freport.pdf",
      fileType: "pdf",
      mimeType: "application/pdf",
    });
  ...
  it("404s when the requested path does not resolve to a file", async () => {
    ...
    await expectHttpStatus(
      handleContextReadRequest(
        ...
        { workbenchId: "workbench-1", userId: "user-1", scheme: "kb", rawPath: "/missing.md" },
      ),
      404,
    );
  });
  ...
  it("surfaces missing object bytes as a clean 404 for binary files", async () => {
    ...
    port.addFile({
      ...
      documentId: "doc-3",
      fileType: "pdf",
      storageUrl: createObjectStorageUrl("context/workbench-1/ghost.pdf"),
      mimeType: "application/pdf",
    });
    ...
    await expectHttpStatus(
      handleContextReadRequest(
        ...
        { workbenchId: "workbench-1", userId: "user-1", scheme: "kb", rawPath: "/ghost.pdf" },
      ),
      404,
    );
  });
  ...
  it("collapses non-404 signed URL failures to a generic 502", async () => {
    ...
    port.addFile({
      ...
      documentId: "doc-4",
      fileType: "pdf",
      storageUrl: createObjectStorageUrl("context/workbench-1/secret.pdf"),
      mimeType: "application/pdf",
    });
    ...
    await expectHttpError(
      handleContextReadRequest(
        ...
        { workbenchId: "workbench-1", userId: "user-1", scheme: "kb", rawPath: "/secret.pdf" },
      ),
