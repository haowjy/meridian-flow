 ...
 * composition boundary where ContextFS becomes project-aware.
 */
import { describe, expect, it } from "vitest";
import { createInMemoryProjectContextPortFactory } from "./index.js";

describe("createInMemoryProjectContextPortFactory", () => {
  it("routes fs1 reads and writes through a project-scoped ContextFS", async () => {
    const factory = createInMemoryProjectContextPortFactory();
    const scope = { userId: "user_1", projectId: "project_1" };
    const port = factory.forProject(scope.projectId, scope.userId);
  ...
  it("keeps project file trees isolated by project", async () => {
    const factory = createInMemoryProjectContextPortFactory();
    const userId = "user_1";
    const first = factory.forProject("project_1", userId);
    const second = factory.forProject("project_2", userId);
  ...
  it("keeps same-project context isolated by user for in-memory test stores", async () => {
    const factory = createInMemoryProjectContextPortFactory();
    const owner = factory.forProject("project_1", "owner");
    const collaborator = factory.forProject("project_1", "collaborator");
