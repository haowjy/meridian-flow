...
export function createPromotionService(deps: PromotionServiceDeps): PromotionService {
  return {
    async promoteArtifact(input): Promise<PromotionResult> {
      ...
      if (!input.workbenchId) return err("invalid_input", "workbenchId is required");
      ...
      const resultsUri = resultsUriForSourcePath(input.provenance.rootThreadId, sourcePath);
      const provenance = {
        ...input.provenance,
