...
describe("promotion service", () => {
  it("promotes a generated PNG with full provenance and lists by workbench", async () => {
    ...
    const listed = await results.listByWorkbench("wb-1");
  ...
  it("skips non-promotable paths via policy", async () => {
    ...
    expect(await results.listByWorkbench("wb-1")).toEqual([]);
  });
});
