...
async function findUploadContextSource(db: Database, workbenchId: string): Promise<string | null> {
...
async function ensureUploadContextSource(db: Database, workbenchId: string): Promise<string> {
  const existing = await findUploadContextSource(db, workbenchId);
  ...
  const raced = await findUploadContextSource(db, workbenchId);
  if (!raced) throw new Error(`Failed to provision thread upload source for ${workbenchId}`);
  return raced;
}
...
export function createDrizzleInternalUploadDocumentStore(
  ...
  return {
    ...
    async createThreadUploadDocument(input: InternalUploadDocumentCreateInput) {
      const contextSourceId = await ensureUploadContextSource(db, input.workbenchId);
