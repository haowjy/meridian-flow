...
describe("LocalObjectStoreAdapter", () => {
  it("stores bytes under a stable object URL and reads them through a signed token", async () => {
    ...
    const put = await store.put(
      "figures/project/document/a.png",
      Buffer.from("image-bytes"),
      "image/png",
    );
    expect(put).toEqual({
      ok: true,
      value: { storageUrl: "object://meridian/figures/project/document/a.png" },
    });

    const signed = await store.getSignedUrl("figures/project/document/a.png");
  ...
  it("expires signed tokens", async () => {
    ...
    await store.put("figures/project/document/a.png", Buffer.from("image-bytes"), "image/png");
    const signed = await store.getSignedUrl("figures/project/document/a.png");
    expect(signed.ok).toBe(true);
    if (!signed.ok) return;
