...
export function createInMemoryPackageStore(
  ...
  return {
    async findPackageInstall(workbenchId, packageName) {
      return state.packages.find(
        (pkg) => pkg.workbenchId === workbenchId && pkg.packageName === packageName,
      );
    },
    ...
    async getAgentWithLinkedSkills(workbenchId, userId, agentSlug) {
      const tx = createTransaction(cloneState(state));
      return resolveAgentSkills(tx, workbenchId, userId, agentSlug);
    },
    dump() {
...
function createTransaction(state: Required<InMemoryPackageStoreSeed>): PackageWriteTransaction {
  return {
    ...
    async findAgentBySlug(workbenchId, slug) {
      return state.agents.find(
        (agent) => agent.workbenchId === workbenchId && agent.slug === slug && agent.enabled,
      );
    },
    ...
    async findSkillBySlug(workbenchId, slug) {
      return state.skills.find(
        (skill) => skill.workbenchId === workbenchId && skill.slug === slug && skill.enabled,
      );
    },
