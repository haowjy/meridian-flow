...
export function describePackageRepositoryConformance(
  ...
  describe(`PackageRepository conformance: ${name}`, () => {
    it("findPackageInstall returns undefined when missing and the row after create", async () => {
      ...
      expect(await repo.findPackageInstall(fixtures.projectId, "pkg-a")).toBeUndefined();
      ...
      const install = await repo.findPackageInstall(fixtures.projectId, "pkg-a");
    ...
    it("commits transaction writes so they are visible after the callback", async () => {
      ...
      await repo.transaction(async (tx) => {
        ...
        expect(await tx.findPackageInstall(fixtures.projectId, "pkg-b")).toBeDefined();
      });

      expect(await repo.findPackageInstall(fixtures.projectId, "pkg-b")).toMatchObject({
        id: installId,
        packageName: "pkg-b",
      });
    });
    ...
    it("getAgentWithLinkedSkills resolves global, builtin, and agent-linked skills", async () => {
      ...
      await repo.transaction(async (tx) => {
        ...
        await tx.createSkill(
          skillInput("global-skill", fixtures.projectId, { type: "principle", isGlobal: true }),
        );
        const linked = await tx.createSkill(
          skillInput("linked-skill", fixtures.projectId, { type: "reference" }),
        );
        await tx.linkAgentSkill({
          agentDefinitionId: agent.id,
          skillId: linked.id,
    ...
    it("listSelectableAgents excludes subagents after Mars import", async () => {
      ...
      const selectable = await repo.transaction((tx) =>
        tx.listSelectableAgents(fixtures.projectId),
      );
...
function skillInput(slug: string, projectId: string | null, meta: Record<string, unknown>) {
  const body = "";
  const fullMeta = {
    name: slug,
    modelInvocable: true,
  ...
  return {
    ...
    sourceType: (projectId ? "package" : "builtin") as "package" | "builtin",
    enabled: true,
  };
}
