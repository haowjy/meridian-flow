// @ts-nocheck
/**
 * Workbench Library inventory: full agent/skill/package summaries for the
...
export async function listWorkbenchLibraryInventory(
  tx: PackageWriteTransaction,
  workbenchId: string,
): Promise<WorkbenchLibraryResponse> {
  const [builtinAgents, workbenchAgents, builtinSkills, workbenchSkills, packageInstalls] =
    await Promise.all([
      tx.listWorkbenchAgentDefinitions(null),
      tx.listWorkbenchAgentDefinitions(workbenchId),
      tx.listWorkbenchSkillDefinitions(null),
      tx.listWorkbenchSkillDefinitions(workbenchId),
  ...
  for (const agent of workbenchAgents) agentsBySlug.set(agent.slug, agent);

  const skillsBySlug = new Map<string, SkillRecord>();
  for (const skill of builtinSkills) skillsBySlug.set(skill.slug, skill);
  for (const skill of workbenchSkills) skillsBySlug.set(skill.slug, skill);
