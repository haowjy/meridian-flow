 ...
 * row still holds the project unique `(projectId, slug)` slot — preview reports
...
async function definitionSlugOccupied(
  ...
  if (kind === "agent") {
    return (await tx.findAgentDefinitionAnyState(projectId, slug)) != null;
  }
  return (await tx.findSkillDefinitionAnyState(projectId, slug)) != null;
}
...
/**
 * Import a Mars package (and its dependencies) into a project.
 *
...
export async function importLocalMarsPackage(
  ...
  try {
    ...
    return await input.repository.transaction(async (tx) =>
      writePackageGraph(tx, input.projectId, graph, {
        sourcePathOverrides:
...
export async function previewLocalMarsPackageImport(input: {
  ...
  try {
    ...
    return await input.repository.transaction(async (tx) =>
      simulateImportPlan(tx, input.projectId, graph, includesSetupInstructions),
    );
  } catch (error) {
    if (isPackageImportError(error)) throw error;
...
export async function previewLocalMarsPackageUpdate(input: {
  ...
  return input.repository.transaction(async (tx) => {
    const installs = await tx.listPackageInstalls(input.projectId);
    ...
    for (const skill of source.skills) {
      ...
      if (await definitionSlugOccupied(tx, input.projectId, "skill", skill.slug)) {
        preview.willKeep.push({ slug: skill.slug, kind: "skill" });
        continue;
      }
      preview.willUpdate.push({ slug: skill.slug, kind: "skill" });
    ...
    for (const agent of source.agents) {
      ...
      if (await definitionSlugOccupied(tx, input.projectId, "agent", agent.slug)) {
        preview.willKeep.push({ slug: agent.slug, kind: "agent" });
        continue;
      }
      preview.willUpdate.push({ slug: agent.slug, kind: "agent" });
...
export async function updateLocalMarsPackage(input: {
  ...
  return input.repository.transaction(async (tx) => {
    ...
    for (const agent of relinkAgents) {
      await replaceAgentLinks(tx, input.projectId, agent.record, agent.parsed, lookup);
    }
...
async function reconcileSkills(
  ...
  for (const skill of source.skills) {
    ...
    if (await definitionSlugOccupied(tx, projectId, "skill", skill.slug)) {
...
async function reconcileAgents(
  ...
  for (const agent of source.agents) {
    ...
    if (await definitionSlugOccupied(tx, projectId, "agent", agent.slug)) {
...
async function resolvePackageGraph(input: {
  ...
  if (await input.repository.findPackageInstall(input.projectId, source.manifest.package.name)) {
...
async function simulateImportPlan(
  ...
  for (const node of graph) {
    ...
    if (await tx.findPackageInstall(projectId, nodeManifest.package.name)) {
...
async function writePackageGraph(
  ...
  for (const node of graph) {
    ...
    if (await tx.findPackageInstall(projectId, manifest.package.name)) {
    ...
    for (const agent of node.source.agents) {
      ...
      if (inserted) await appendAgentLinks(tx, projectId, inserted, agent, lookup);
    }
  }
...
async function appendAgentLinks(
  ...
  for (const link of await desiredSkillLinks(tx, projectId, agent, parsed, lookup)) {
...
async function replaceAgentLinks(
  ...
  await tx.replaceAgentSkillLinks(
    agent.id,
    await desiredSkillLinks(tx, projectId, agent, parsed, lookup),
  );
}
...
async function desiredSkillLinks(
  ...
  for (const skillSlug of stringsAt(parsed.meta.skills)) {
    ...
    const skill =
      lookup.skillsBySlug.get(skillSlug) ?? (await tx.findSkillBySlug(projectId, skillSlug));
    if (skill) {
      links.push({
...
function localPackageInstallUpdate(
  ...
  PackageInstallRecord,
  "id" | "projectId" | "packageName" | "sourceCommitSha" | "sourceRef"
> {
  return {
