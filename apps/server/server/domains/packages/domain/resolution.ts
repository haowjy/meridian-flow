// @ts-nocheck
/**
 * Agent skill-resolution algorithm: merges builtin, user, global-workbench, and
 ...
 * builtin skills, user skills, global workbench skills, agent-linked skills,
 ...
 *
 *   1. builtin (global/system skills, workbenchId IS NULL)
 *   2. user-installed (per-user skills)
 *   3. workbench-global (per-workbench skills where isGlobal=true)
...
export async function resolveAgentSkills(
  ...
  const projectAgent = await tx.findAgentBySlug(workbenchId, agentSlug);
  const agent = projectAgent ?? builtinAgent;
  const merged = new Map<string, ResolvedSkill>();

  for (const skill of await tx.listWorkbenchSkills(null)) {
  ...
  for (const skill of await tx.listWorkbenchSkills(workbenchId)) {
    ...
    for (const link of await tx.listAgentSkillLinks(agent.id)) {
      ...
      if (linkedSkill) {
        addResolvedSkill(merged, linkedSkill.slug, {
          skill: linkedSkill,
          layer: linkedSkill.workbenchId ? "workbench" : "builtin",
