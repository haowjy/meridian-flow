...
import { listWorkbenchLibraryInventory } from "./workbench-library.js";
...
describe("definition-editing", () => {
  ...
  it("clears edited state in library inventory after description save and restore original", async () => {
    ...
    await store.transaction(async (tx) => {
      const saved = await saveAgentDefinition(tx, workbenchId, "segmentation", {
        body: pristineBody,
      ...
      const restored = await restoreAgentDefinitionOriginal(tx, workbenchId, "segmentation");
      ...
      const library = await listWorkbenchLibraryInventory(tx, workbenchId);
  ...
  it("patches modelInvocable without marking the definition edited", async () => {
    ...
    await store.transaction(async (tx) => {
      const patched = await patchAgentSkillLink(tx, workbenchId, "segmentation", "segment", {
