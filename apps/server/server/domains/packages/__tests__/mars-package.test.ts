  ...
  it("auto-updates pristine items and skips locally modified items unless forced", async () => {
    await withTempDir(async (dir) => {
      ...
      await store.transaction(async (tx) => {
        const agent = await tx.findAgentBySlug("project-1", "agent-one");
...
`,
  ...
  it("does not retarget package links to colliding project records", async () => {
    await withTempDir(async (dir) => {
      await writeMarsFixture(dir, { name: "pkg-one" });
      const existingSkill = skillRecord("skill-one", "project-1", { type: "reference" });
  ...
  it("keeps subagent metadata local when project agent slugs collide", async () => {
    await withTempDir(async (dir) => {
      await writeMarsFixture(dir, { name: "pkg-one" });
      await writeFile(
        path.join(dir, "agents", "agent-one.md"),
...
`,
      ...
      const existingAgent = agentRecord("agent-two", "project-1");
...
describe("resolution hierarchy", () => {
  it("merges builtin, user, global, and agent-linked skills ordered by type", async () => {
    ...
    const globalProject = skillRecord("global-skill", "project-1", {
      type: "principle",
      isGlobal: true,
    });
    const linked = skillRecord("linked-skill", "project-1", { type: "reference" });
    const agent = agentRecord("agent-one", "project-1");
    const store = createInMemoryPackageStore({
      agents: [agent],
      skills: [builtin, globalProject, linked],
...
function agentRecord(slug: string, projectId: string | null): AgentDefinitionRecord {
  const record = {
    id: `agent-${slug}`,
    projectId,
    slug,
    ...
    originalContentChecksum: null,
    sourceType: projectId ? "package" : "builtin",
    enabled: true,
