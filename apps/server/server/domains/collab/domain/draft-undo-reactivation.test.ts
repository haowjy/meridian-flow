...
async function replaceLiveBlockText(
  ...
  await scenario.coordinator.withDocument(DOC_ID, async (doc) => {
    ...
    const block = scenario.model.getBlocks(handle)[blockIndex];
    if (!block) throw new Error(`expected live block ${blockIndex}`);
    const before = Y.encodeStateVector(doc);
...
describe("draft undo and reactivation", () => {
  ...
  it("acceptance: whole-draft apply fails closed after a reactivated insert cannot be placed", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    const draft = await scenario.store.createActiveDraft({
      documentId: DOC_ID,
      threadId: THREAD_ID,
    ...
    expect(fullApply).toMatchObject({ status: "cannot_place", draftId: draft.id });
    expect(normalizeMarkdown(await liveMarkdown(scenario))).toBe("Writer replacement.");
  });
  ...
  it("acceptance: duplicate-content clean redo fails closed without guessing from position", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    ...
    await scenario.store.appendUpdate({
      draftId: draft.id,
      updateData: await updateFromMarkdownOverLive(scenario, "Echo.\n\nNew.\n\nEcho."),
  ...
  it("acceptance: unique-content clean redo still re-applies single-copy after undo", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    ...
    expect(reaccept).toMatchObject({ status: "applied", draftId: draft.id });
    const live = normalizeMarkdown(await liveMarkdown(scenario));
    expect(live).toBe(finalMarkdown);
    expect(live.match(/New\./g)).toHaveLength(1);
  });

  it("acceptance: reactivated duplicate decoy anchor fails closed without drifting", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    await replaceLiveMarkdown(scenario, "Echo.\n\nEcho.\n\nTail.");
    const draft = await scenario.store.createActiveDraft({
      documentId: DOC_ID,
    ...
    expect(normalizeMarkdown(await liveMarkdown(scenario))).toBe("Echo.\n\nEcho.\n\nTail.");

    await prependLiveMarkdownBlock(scenario, "Echo.");
    const unchanged = normalizeMarkdown(await liveMarkdown(scenario));
    expect(unchanged).toBe("Echo.\n\nEcho.\n\nEcho.\n\nTail.");
  ...
  it("acceptance: reactivated replaceAll preserves writer edits to untouched equal blocks", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    await replaceLiveMarkdown(scenario, "Alpha.\n\nBeta.\n\nGamma.");
    const draft = await scenario.store.createActiveDraft({
      documentId: DOC_ID,
    ...
    await scenario.store.appendUpdate({
      draftId: draft.id,
      updateData: await updateFromMarkdownOverLive(scenario, "Alpha.\n\nBeta.\n\nGamma revised."),
      updateKind: "replaceAll",
      actorTurnId: TURN_A,
    });
    ...
    await replaceLiveBlockText(scenario, 1, "Beta writer edit.");
  ...
  it("acceptance: reactivated replaceAll still fails closed on writer edits to changed block", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    await replaceLiveMarkdown(scenario, "Alpha.\n\nBeta.\n\nGamma.");
    const draft = await scenario.store.createActiveDraft({
      documentId: DOC_ID,
    ...
    await replaceLiveBlockText(scenario, 2, "Gamma writer edit.");
    const unchanged = normalizeMarkdown(await liveMarkdown(scenario));
  ...
  it("acceptance: whole-doc replaceAll move draft re-applies single-copy after undo", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    await replaceLiveMarkdown(scenario, "Alpha.\n\nBeta.\n\nGamma.");
    const originalBlocks = await liveBlockSummaries(scenario);
    const draft = await scenario.store.createActiveDraft({
    ...
    expect(normalizeMarkdown(await liveMarkdown(scenario))).toBe(finalMarkdown);

    const undo = await scenario.service.undoAcceptDraft({
      documentId: DOC_ID,
      threadId: THREAD_ID,
      draftId: draft.id,
      userId: USER_ID,
    });
    expect(undo).toMatchObject({ status: "reactivated", draftId: draft.id });
    expect(normalizeMarkdown(await liveMarkdown(scenario))).toBe("Alpha.\n\nBeta.\n\nGamma.");

    const restoredBlocks = await liveBlockSummaries(scenario);
    expect(restoredBlocks.map((block) => block.text)).toEqual(
      originalBlocks.map((block) => block.text),
    );
    expect(restoredBlocks[0]?.id).not.toBe(originalBlocks[0]?.id);
    expect(restoredBlocks[1]?.id).not.toBe(originalBlocks[1]?.id);
    expect(restoredBlocks[2]?.id).not.toBe(originalBlocks[2]?.id);

    const preview = await scenario.preview.previewDraft({
      documentId: DOC_ID,
      draftId: draft.id,
    });
    const unconfirmedReaccept = await scenario.service.acceptDraft({
      documentId: DOC_ID,
      threadId: THREAD_ID,
      draftId: draft.id,
      userId: USER_ID,
    ...
    const reaccept =
      unconfirmedReaccept.status === "overlap"
        ? await scenario.service.acceptDraft({
            documentId: DOC_ID,
            threadId: THREAD_ID,
            draftId: draft.id,
            userId: USER_ID,
            draftRevisionToken: preview.draftRevisionToken,
            confirmedLiveRevisionToken: unconfirmedReaccept.liveRevisionToken,
            confirmOverlap: true,
          })
        : unconfirmedReaccept;
    ...
    expect(live.match(/Beta-revised\./g)).toHaveLength(1);
  ...
  it("acceptance: moved-content destination with only far anchors fails closed without drifting", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    await replaceLiveMarkdown(scenario, "A.\n\nMOVE.\n\nC.\n\nD.\n\nE.");
    const draft = await scenario.store.createActiveDraft({
      documentId: DOC_ID,
    ...
    await scenario.store.appendUpdate({
      draftId: draft.id,
      updateData: await updateFromMarkdownOverLive(scenario, "A.\n\nC.\n\nD.\n\nMOVE.\n\nE."),
      updateKind: "replaceAll",
      actorTurnId: TURN_A,
    });
    ...
    await replaceLiveBlockText(scenario, 2, "C writer.");
    await replaceLiveBlockText(scenario, 3, "D writer.");
    await replaceLiveBlockText(scenario, 4, "E writer.");
    const unchanged = normalizeMarkdown(await liveMarkdown(scenario));
  ...
  it("acceptance: reactivated whole-draft brand-new insert still places between matched neighbours", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    await replaceLiveMarkdown(scenario, "A.\n\nC.");
    const draft = await scenario.store.createActiveDraft({
      documentId: DOC_ID,
  ...
  it("acceptance: reactivated replaceAll cannot place over writer-edited touched block", async () => {
    ...
    await replaceLiveBlockText(scenario, 1, "Beta writer edit.");
    const afterWriterEdit = normalizeMarkdown(await liveMarkdown(scenario));
    expect(afterWriterEdit).toBe("Alpha.\n\nBeta writer edit.\n\nGamma.");

    const preview = await scenario.preview.previewDraft({
      documentId: DOC_ID,
      draftId: draft.id,
    });
    const overlap = await scenario.service.acceptDraft({
      documentId: DOC_ID,
      threadId: THREAD_ID,
      draftId: draft.id,
      userId: USER_ID,
    ...
    expect(overlap).toMatchObject({ status: "overlap", draftId: draft.id });
    ...
    const reaccept = await scenario.service.acceptDraft({
      ...
      confirmedLiveRevisionToken:
        overlap.status === "overlap" ? overlap.liveRevisionToken : preview.liveRevisionToken,
      confirmOverlap: true,
    });

    expect(reaccept).toMatchObject({ status: "cannot_place", draftId: draft.id });
    expect(normalizeMarkdown(await liveMarkdown(scenario))).toBe(afterWriterEdit);
  });
  ...
  it("fails closed for later reactivated duplicate inserts without same-content anchor lookup", async () => {
    const scenario = await createScenarioWithRealAcceptUndo();
    await replaceLiveMarkdown(scenario, "Echo.\n\nEcho.\n\nEcho.");
    const draft = await scenario.store.createActiveDraft({
      documentId: DOC_ID,
    ...
    const childAccept =
      childUnconfirmed.status === "closure_confirmation_required"
        ...
        : childUnconfirmed;
    expect(childAccept).toMatchObject({ status: "cannot_place" });
    expect(normalizeMarkdown(await liveMarkdown(scenario))).toBe(
      "Echo.\n\nEcho.\n\nEcho.\n\nEcho.",
    );
  });
