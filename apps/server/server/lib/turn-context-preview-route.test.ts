...
function seedPackage(workbenchId: string) {
  const skill = skillRecord(workbenchId, "skill-one");
...
function previewDeps(input: {
  ...
  workbenchRepo: ReturnType<typeof createInMemoryWorkbenchRepository>;
  modelRequestDebug: ReturnType<typeof createInMemoryModelRequestDebugStore>;
  workbenchId: string;
}) {
  const packageRepository = seedPackage(input.workbenchId);
  ...
  return {
    repos: input.repos,
    workbenchRepo: input.workbenchRepo,
