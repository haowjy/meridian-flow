 ...
 */
import type { WorkRepository } from "../domains/workbenches/index.js";
...
export interface ResolveWorkIdArgs {
  ...
  /** Title for the default work when one must be created (e.g. workbench title). */
  defaultTitle?: string;
}
 ...
 * - A primary thread attaches to the workbench's default work, creating one if
 *   the workbench has none yet. This keeps every primary thread grouped under a
...
export async function resolveWorkIdForThread(
  ...
  const work = await deps.workRepo.ensureDefaultForWorkbench(args.workbenchId, args.defaultTitle);
  return work.id;
}
