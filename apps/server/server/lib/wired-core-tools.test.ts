...
import { createInMemoryContextPortFactory } from "../domains/context/index.js";
...
describe("createWiredCoreToolRegistrations", () => {
  ...
  it("executes search through the tool executor", async () => {
    const repos = createInMemoryRepositories();
    const thread = await repos.threads.create({ userId: "user_1", workbenchId: "project_1" });
    const portFactory = createInMemoryContextPortFactory();
    const port = portFactory.forWorkbench(thread.workbenchId, thread.userId);
    await port.write("kb://protocols/blot.md", "western blot needle\nwash membrane", {
      origin: { type: "system" },
    });
    await port.write("work://notes.md", "needle outside the knowledge base", {
      origin: { type: "system" },
    });
  ...
  it("records document touches after reads and writes", async () => {
    const repos = createInMemoryRepositories();
    const thread = await repos.threads.create({ userId: "user_1", workbenchId: "project_1" });
  ...
  it("runs ask_user through the orchestrator checkpoint round-trip", async () => {
    ...
    const thread = await repos.threads.create({ userId: "user-1", workbenchId: "workbench-1" });
