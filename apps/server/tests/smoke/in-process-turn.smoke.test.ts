...
import { createInMemoryWorkbenchRepository } from "../../server/domains/workbenches/index.js";
...
describe("smoke: in-process turn", () => {
  it("runs read through wired ContextPort tools and persists the turn lifecycle", async () => {
    ...
    const workbench = await workbenchRepo.create({ userId: "smoke-user", title: "Smoke" });
    const thread = await repos.threads.create({ userId: "smoke-user", workbenchId: workbench.id });
    const contextPorts = createInMemoryContextPortFactory();
    const writeResult = await contextPorts
      .forWorkbench(workbench.id, workbench.userId)
