...
Figma runs two distinct shells: a **file browser** (sidebar + thumbnail grid) and a **canvas project** (left structure panel, infinite center canvas, right inspector, bottom tool rail). UI3 adds **minimize-UI** — sidebars collapse to floating pills so the canvas owns most of the viewport. Creation is multi-path (Create menu, sidebar New file, AI prompt bar, double-click resume). Density is bimodal: airy marketing grid on Home, compact 28–32px list rows and icon-only tool chrome in the editor.
...
1. **Two shells, one product** — launcher and project are different layout contracts.
2. **Three-column project** on desktop — structure left, canvas center, inspector right; tools on a fourth band (bottom rail).
...
2. **Open = hard context switch** — launcher → project replaces shell.
3. **On mobile web, default to minimized project** if three columns cannot fit.
...
1. **Bimodal density** — launcher breathes; project panels compress.
...
| **Panels** | Left structure + right inspector; resize handles | Chat single column; Context/KB routes stubby | Three-zone project: structure + viewer + inspector/dock |
...
1. Project shell with resizable panels + **minimize mode** (trigger below ~1024px or user toggle).
...
| Meridian Flow Context/KB and spatial viewers not built in phase-1 | Project comparison is structural; portless URL used for rail/chat observations only |
