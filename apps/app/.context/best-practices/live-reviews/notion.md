...
| Meridian Flow project project | `…/project/{id}` |
...
Meridian Flow today is **conversation/project-first**: home uses a ~279px projects sidebar and the same ~720px centered column; in-project uses a slim **icon rail** plus card-based project (not a block editor). Mobile collapses navigation into a **TopBar hamburger → drawer** with project list drill-down and destination links — structurally similar to Notion’s overlay nav, but without page-tree nesting or block-level editing.
...
|-----------|-------------------|-------------------|-------------------|
| **Layout** | Sidebar + centered 720px block editor | Home: sidebar + 720px column; Project: icon rail + card project | Adopt **fixed content column** for long-form surfaces (notes, reports). Keep icon rail for project; add optional **doc column** when block/markdown editing ships. |
