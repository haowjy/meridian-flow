...
| `extensions/theme-defaults/themes/dark_modern.json` | Workbench `colors` map |
...
|-------|------|-------------|
| **Workbench colors** | Dotted IDs (`editor.background`, `sideBar.foreground`, `button.background`) | UI via CSS variables |
...
`uiTheme` selects base scheme class on the workbench root and drives registry default resolution (light vs dark vs high-contrast).
...
| CSS emission | `browser/colorThemeCss.ts` → `generateColorThemeCSS()` | Iterates registry, emits all resolved colors as vars on `.monaco-workbench { … }` |
| Runtime apply | `browser/workbenchThemeService.ts` → `applyTheme()` | Injects `<style>` sheet, toggles `vs`/`vs-dark`/`hc-*` classes |

Components and Monaco theming participants read `var(--vscode-*)` or call `theme.getColor('editor.background')`.

### Runtime load / switch

Flow in `workbenchThemeService.ts`:
...
5. **React** — configuration listener reloads when `workbench.colorCustomizations` changes.

Settings in `themeConfiguration.ts`: `workbench.colorTheme`, `window.autoDetectColorScheme`, preferred dark/light/HC themes.
...
| Setting | Overrides |
|---------|-----------|
| `workbench.colorCustomizations` | Workbench color IDs; supports theme-scoped keys like `"[Dark Modern]": { "editor.background": "#000" }` |
...
| **Theme swap = class + var override** | `.monaco-workbench.vs-dark` + regenerated vars | Planned: `.dark { --background: … }` on `html` (`globals.css` `@custom-variant dark`) |
| **Preset themes as data files** | JSON `colors` maps | Could be JSON → apply to `:root` without VS Code's registry |
| **User overrides as a patch object** | `workbench.colorCustomizations` | `localStorage` patch merged over active preset |
...
None of these expose VS Code-grade per-token workbench customization; they stop at scheme switching.
...
|--------------------|-------------------|
| **~253 workbench color IDs** | Meridian Flow has **~64** semantic vars; 4× smaller surface, shadcn-shaped |
...
apps/app/src/styles/globals.css
  ...
  ├─ :root workbench-only   ← composer fade, answer max-width
...
| VS Code workbench key | Meridian Flow equivalent (approx.) |
|-----------------------|----------------------------|
...
| **Phase 4 — VS Code theme import** | **M–L** | 1–3 weeks: map subset of `workbench.colorCustomizations` keys → Meridian Flow vars; ignore tokenColors; heuristic mapping table |
...
2. **VS Code splits workbench colors from syntax token colors** — two pipelines (CSS vars vs TextMate/semantic). Meridian Flow's UI can theme-swap with CSS alone; markdown/code highlighting is a **separate** Shiki decision and blocks "full IDE theme" parity.
...
| Source | Path |
|--------|------|
| VS Code theme service | `~/.meridian/ref/vscode/src/vs/workbench/services/themes/browser/workbenchThemeService.ts` |
| VS Code theme data / load | `~/.meridian/ref/vscode/src/vs/workbench/services/themes/common/colorThemeData.ts` |
| VS Code CSS generation | `~/.meridian/ref/vscode/src/vs/workbench/services/themes/browser/colorThemeCss.ts` |
| VS Code color registry | `~/.meridian/ref/vscode/src/vs/platform/theme/common/colorUtils.ts`, `colors/*.ts` |
| VS Code extension point | `~/.meridian/ref/vscode/src/vs/workbench/services/themes/common/themeExtensionPoints.ts` |
