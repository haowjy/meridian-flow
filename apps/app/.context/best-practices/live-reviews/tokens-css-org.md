...
apps/app/src/styles/globals.css
  ...
  ├─ :root { workbench-only } ← app-specific vars (composer fade, answer max-width)
...
|-------|------|--------------|
| `@meridian/design-tokens` | Shared Tailwind v4 `@theme` palette, shadows, gradients, status colors, type scale, radii; motion duration | `@utility`, workbench layout |
| `globals.css` `@theme` | Workbench-only container widths and shell shadows | Shared palette/type/radius aliases |
...
- **Tailwind source:** `warm-paper.css` declares the real values directly in `@theme`; app-side `@theme` is only for workbench-specific tokens.
...
|------|-------|------|---------|
| **1 — Semantic tokens** | `warm-paper.css` + workbench-only `@theme` in `globals.css` | Cross-surface values two+ components must agree on | `--container-chat-column`, `text-answer`, `shadow-card` |
...
2. New visual concept in **≥2 places** → new Tier 1 token in `warm-paper.css` (shared) or workbench `@theme`/`:root` (app-only), then consume the token directly.
...
These encode the viewport-locked workbench shell documented in DESIGN.md § Layout:
...
1. **Keep the three-file split.** Shared palette stays in `@meridian/design-tokens`; never add `@utility` or `@theme` there. Workbench-only vars (`--chat-scroll-fade-size`, `--answer-max-width`) stay in `globals.css` `:root` until a second surface (`@meridian/web`) needs them — then promote to the package.
...
| Workbench-only computed vars (fade gradients) | `apps/app/src/styles/globals.css` `:root` |
...
| `@meridian/web` parity unchecked | Assumption: web imports same `warm-paper.css`; verify before promoting workbench vars to package |
