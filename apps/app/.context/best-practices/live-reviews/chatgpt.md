...
1. **Preserve Meridian Flow’s richer home** (recent projects, agent packages) — ChatGPT’s bare landing is optimized for consumer chat, not research project entry. Don’t strip home to a single prompt.
...
| Home landing | Single prompt | Prompt + recent projects + agent packages | **Intentional** project entry |
