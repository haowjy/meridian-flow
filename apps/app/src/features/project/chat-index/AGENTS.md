...
the same `recencyGroups` and `RecencyGroupLabel`, quiet failure. The whole pane
scrolls as one; the `[Search] [All | Favorites]` row sticks once scrolled past.
Chats grow without bound, so the list pages from the server, searches there
