/**
 * Placement tests for the project surface layout model.
 *
...
describe("project surface placement", () => {
  ...
  it("drops context-files during v2→v3 migration (value must be harvested first)", () => {
    ...
    // PROJECT_SURFACE_IDS, so the key falls away without error. This is
    ...
    // context-files is gone — it's not in PROJECT_SURFACE_IDS anymore.
    // The project store is correct to drop it; the new context-files-panel
