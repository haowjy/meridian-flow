...
- A confirmed-empty list still shows skeletons for one round trip when the query
  ...
  query is invalidated (`getQueryState(accountQueryKeys.recentDocuments())`),
  and leave a plain stale refetch on the first-run copy.
- A cold-cache return can still paint the first-run copy for a beat: if the
  ...
  accountQueryKeys.recentDocumentsRoot })` before the invalidate, at both call
  sites, closes it.
...
- Scratch and uploads rows would open onto the "Viewing chat resources is not
  ...
  such tab and the create recorder skips those schemes, so none is recorded
  today; omit those schemes from the list if one ever is.
