 ...
 * prefs out of the shared project surface-layout store, the seed MUST read
 * the legacy blob BEFORE the project v2→v3 migration rewrites it (the
 ...
 * Matches the shape the project store's `migrate` function expects:
...
describe("context-files-store upgrade path", () => {
  ...
  it("preserves context-files prefs when seed runs BEFORE project migrate (correct order)", () => {
    const { store, ls } = createMockLocalStorage();
    vi.stubGlobal("localStorage", ls);

    // Pre-upgrade state: v2 project blob with saved files-panel prefs.
    store.set(
      "meridian:project-surface-layout",
    ...
    // 2. Simulate project v2→v3 migration (which happens after seed).
    //    Prove the migration drops context-files — correct only because the
    //    seed already harvested the value.
    const v2Raw = store.get("meridian:project-surface-layout");
    expect(v2Raw).toBeDefined();
    if (!v2Raw) throw new Error("missing legacy project prefs");
  ...
  it("LOSES context-files prefs when project migrates first (wrong order — guards the bug)", () => {
    const { store, ls } = createMockLocalStorage();
    vi.stubGlobal("localStorage", ls);
    ...
    // WRONG order: simulate project v2→v3 migration happening first.
    // The migration rewrites the blob without context-files (just as zustand
    // persist does when the project store rehydrates before the seed).
    const v2Raw = store.get("meridian:project-surface-layout");
    expect(v2Raw).toBeDefined();
    if (!v2Raw) throw new Error("missing legacy project prefs");
    const v2Parsed = JSON.parse(v2Raw);
...
describe("seedContextFilesPanelFromLegacy", () => {
  ...
  it("no-ops without throwing when the legacy project key is absent", () => {
  ...
  it("no-ops without throwing when the legacy JSON is corrupt", () => {
    ...
    store.set("meridian:project-surface-layout", "not valid json{{{");
  ...
  it("handles legacy blob with prefs but no context-files key", () => {
    ...
    store.set("meridian:project-surface-layout", JSON.stringify(v1Blob));
