# features/project — Desktop workbench shell

The authenticated project workbench: one persistent multi-panel desktop surface
...
Design intent lives in [`DESIGN.md` § Workbench shell](../../../../../../DESIGN.md);
...
shared `WorkbenchView` selects `mobile/MobileWorkbench` only through the phone
capability predicate in `usePhoneShell()`; desktop `WorkbenchShell` remains the
...
`layout/SlotGrid.tsx` renders the whole workbench as a **single flat CSS grid**.
...
in the same `_authenticated.tsx` effect behind the same workbench gate.

`layout/surface-prefs-store.ts` is the **device-local** chrome-prefs store
(Zustand `persist`, localStorage key `meridian:workbench-surface-layout`,
...
2. `routes/_authenticated.tsx` calls `useWorkbenchSurfacePrefsStore.persist
   .rehydrate()` (synchronous — localStorage) then `setHydrated()` immediately.
3. **`ProjectView` gates the whole `DesktopWorkbench` mount on `_hydrated`** so it
   mounts exactly once against final persisted prefs (at most one frame, no
   visible flash).

Two rules keep this stable:

- **Gate at the parent (`ProjectView`), never inside `DesktopWorkbench`.** A
...
- **`DesktopWorkbench` takes a single merged layout subscription**
  (`useWorkbenchLayout`). Do not add a second whole-prefs subscription — that
