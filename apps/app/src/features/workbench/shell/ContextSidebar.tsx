// @ts-nocheck
/**
 * ContextSidebar — right-side workbench rail summarizing work context sections
 * and deferred artifact surfaces. It is visual chrome only; data ownership
 * stays with the workbench/context feature hooks.
 */
 ...
 *
 * Three sections, labels locked by the workbench design brief:
 *
 ...
 *   3. **Results** — promoted artifacts the agent produced (workbench-scoped,
...
export type ContextSidebarProps = {
  ...
  /** Active workbench; powers the Results section (workbench-scoped, not thread-scoped). */
  workbenchId: string | null;
  onClose: () => void;
};

export function ContextSidebar({ threadId, workbenchId, onClose }: ContextSidebarProps) {
  const uploads = useThreadUploads(threadId);
  const recent = useThreadRecentDocuments(threadId);
  // Results live at the workbench scope (artifact persistence outlives any
  // single chat), so the rail tracks `workbenchId` independently of the
