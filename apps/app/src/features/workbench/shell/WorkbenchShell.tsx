// @ts-nocheck
/**
 * WorkbenchShell — destination-agnostic desktop layout mechanics.
 *
...
import { DESKTOP_WORKBENCH_SLOTS, getDesktopGridTemplate } from "../layout/desktop-layout";
      ...
      style={
        {
          ...
          "--workbench-left-handle-width": "0px",
          "--workbench-main-min-width": `${mainMinWidth ?? 0}px`,
          "--workbench-dock-handle-width": "0px",
          "--workbench-dock-width": `${dockWidth}px`,
        } as CSSProperties
      }
    >
      {routePaneArea ? (
      ...
      {leftOccupant ? (
        <div className="relative z-20 min-h-0 overflow-visible" style={{ gridArea: "left-resize" }}>
          <SurfaceResizeHandle
            gridRef={gridRef}
            cssVariableName="--workbench-left-width"
      ...
      {dockOccupant ? (
        <div className="relative z-20 min-h-0 overflow-visible" style={{ gridArea: "dock-resize" }}>
          ...
          <ResizeHandle
            gridRef={gridRef}
            cssVariableName="--workbench-dock-width"
            widthPx={dockWidth}
...
function visibleOccupantOf(
  layout: SurfaceLayoutMap,
  slot: DesktopWorkbenchSlotId,
): SurfaceId | null {
...
function occupantOf(layout: SurfaceLayoutMap, slot: DesktopWorkbenchSlotId): SurfaceId | null {
  return WORKBENCH_SURFACE_IDS.find((surfaceId) => layout[surfaceId].slot === slot) ?? null;
}
