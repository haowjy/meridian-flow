 ...
 * promoted artifacts ("Results") for the active workbench, with type icon,
...
import { useWorkbenchResults } from "@/client/query/useWorkbenchResults";
...
export type ResultsRailSectionProps = {
  ...
  onOpenResult: (result: WorkbenchResultItem) => void;
};

export type ResultsRailModel = {
  status: ReturnType<typeof useWorkbenchResults>;
  /** `null` hides the count while loading/error/disabled. */
  count: number | null;
};

export function useResultsRailModel(workbenchId: string | null): ResultsRailModel {
  const status = useWorkbenchResults(workbenchId);
  ...
  return (
    <div className="flex flex-col gap-0.5">
      {status.status === "disabled" ? (
        <EmptyHint>
          <Trans>Open a workbench to see its results.</Trans>
        </EmptyHint>
...
export function ResultsRailSection({ workbenchId, onOpenResult }: ResultsRailSectionProps) {
  const model = useResultsRailModel(workbenchId);
  ...
  return (
    <Section
      ...
      defaultOpen
    >
      <ResultsRailBody workbenchId={workbenchId} model={model} onOpenResult={onOpenResult} />
    </Section>
  );
}
 ...
 * `/project workspace/path.ext` and bare URI shapes (`results://workbench/foo.ext`),
 * so it gets its own test alongside the rail render.
 */

export function displayName(result: WorkbenchResultItem): string {
...
function formatResultDetail(result: WorkbenchResultItem): string {
