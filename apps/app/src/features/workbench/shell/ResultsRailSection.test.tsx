...
vi.mock("@/client/query/useWorkbenchResults", () => ({
  useWorkbenchResults: vi.fn(),
}));

vi.mock("@/client/query/useWorkbenchAgents", () => ({
  useWorkbenchAgents: vi.fn(() => ({
    status: "disabled",
    agents: null,
    data: null,
    isError: false,
...
const mockedUseResults = useWorkbenchResults as unknown as ReturnType<typeof vi.fn>;

function makeResult(overrides: Partial<WorkbenchResultItem> = {}): WorkbenchResultItem {
  return {
    id: "result_1",
    workbenchId: "wb-1",
...
describe("Results rail rendering", () => {
  it("renders a result row with the producing agent slug visible", () => {
    ...
    const html = renderToStaticMarkup(
      <ResultsRailSection workbenchId="wb-1" onOpenResult={() => undefined} />,
    );
  ...
  it("renders a disabled hint when there is no workbench yet", () => {
    mockedUseResults.mockReturnValue({
      status: "disabled",
      data: null,
      results: null,
    ...
    const html = renderToStaticMarkup(
      <ResultsRailSection workbenchId={null} onOpenResult={() => undefined} />,
    );

    expect(html).toContain("Open a workbench");
  });
