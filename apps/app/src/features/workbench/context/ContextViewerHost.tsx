 ...
 *
 * Fetches the active file through `useWorkbenchContextRead` (the signed-URL
...
import { useWorkbenchContextRead } from "@/client/query/useWorkbenchContextRead";
...
function ContextViewerContent({
  ...
  const read = useWorkbenchContextRead(workbenchId, tab.scheme, tab.path);
  if (tab.editable) {
    return (
