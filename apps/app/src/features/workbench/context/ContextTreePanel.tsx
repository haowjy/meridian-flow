...
export type ContextTreePanelProps = {
  ...
  activeScheme: WorkbenchContextTreeScheme | null;
  ...
  onSelectFile: (scheme: WorkbenchContextTreeScheme, file: ContextFile) => void;
 ...
 * `+` menu. Sections fetch lazily — `useWorkbenchContextTree` only runs once
...
export function ContextTreePanel({
  ...
  function focusFolderFor(scheme: WorkbenchContextTreeScheme): string {
...
function SchemeSection({
  ...
  const { tree, isError, isFetching } = useWorkbenchContextTree(workbenchId, scheme, {
    enabled: isOpen,
  });
  ...
  useEffect(() => {
    ...
    setPendingOpenPath(null);
  }, [pendingOpenPath, tree, openTab, workbenchId, scheme]);
...
function FileRow({
  ...
  scheme: WorkbenchContextTreeScheme;
  active: boolean;
  onSelect: (scheme: WorkbenchContextTreeScheme, file: ContextFile) => void;
}) {
  const indent = depth * 14;
  return (
    <li>
...
function CreateRow({
  ...
  const mutation = useCreateContextEntry(workbenchId, scheme);
