 ...
 * (`"kb" | "work" | "user" | "fs1"`) match the server's `WorkbenchContextTreeScheme`
 ...
 */
export const CONTEXT_SCHEMES: readonly WorkbenchContextTreeScheme[] = ["kb", "user", "work", "fs1"];
 ...
 */
export function schemeLabel(scheme: WorkbenchContextTreeScheme): string {
  switch (scheme) {
    case "kb":
 ...
 */
export function schemeIcon(scheme: WorkbenchContextTreeScheme): LucideIcon {
  switch (scheme) {
    case "kb":
      return BookOpen;
    case "user":
