 ...
 * the shared workbench surface-prefs store. This panel is NEVER placed in a
 ...
 * so it doesn't entangle the workbench shell's hydration gate.
 *
 * Key decision: no separate `_hydrated` gate. The store is consumed only inside
 * `ContextViewer`, which only mounts inside `DesktopWorkbench`, which is ALREADY
 * gated on the workbench store's `_hydrated`. A second gate would be redundant
 * complexity. The only requirement is that this store is rehydrated (and seeded
 * from legacy) in `_authenticated.tsx` BEFORE the workbench `setHydrated()` call.
 */
 ...
 * shared workbench store key and materialize it under the new dedicated key.
 *
...
export function seedContextFilesPanelFromLegacy(): void {
  ...
  try {
    const raw = localStorage.getItem("meridian:workbench-surface-layout");
    if (!raw) return;
