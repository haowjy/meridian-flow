...
/**
 * Flatten a workbench's primary threads into `ChatRow[]`, joined to Work labels.
 *
 ...
 */
export function useChatsOverview(workbenchId: string): ChatsOverview {
  const { primaryThreads, threadsLoaded } = useWorkbenchThreadGroups(workbenchId);
