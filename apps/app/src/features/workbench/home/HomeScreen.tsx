 ...
 * workbench. Ports the P5 proto's overview table, wired to real data.
 *
 ...
 * top bar is owned by `PaneHeader` in `WorkbenchView`; this renders body only.
 */
...
export function HomeOverviewBody({
  ...
  const { rows, workCount, loaded } = useChatsOverview(workbenchId);
        ...
        <header className={cn("flex flex-col gap-4", headerClassName)}>
          <div className="flex min-w-0 flex-col gap-2">
            ...
            <p className="max-w-[58ch] text-[13.5px] leading-6 text-ink-muted">
              <Trans>
                One surface for every chat in this workbench. Jump into any conversation, or start a
                new one.
              </Trans>
            </p>
          </div>
