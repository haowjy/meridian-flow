# workbench/mobile — Phone workbench shell

The phone workbench is a **sibling shell** of the desktop workbench, not a
responsive branch inside `WorkbenchShell`. `WorkbenchView` selects it through
...
The workbench route owns all phone navigation state:

| Param | Meaning |
|---|---|
| `?screen=` | Active primary workbench destination: `home`, `chat`, or `context`. |
...
`features/workbench/shell/screens.ts` has one primary destination registry:
...
`mobile-workbench-document-host`, retains `[]` when no editable document is open,
...
WorkbenchView
  └─ HydratedWorkbench
       ├─ usePhoneShell() === true  → MobileWorkbench
       └─ usePhoneShell() === false → DesktopWorkbench
...
- The trailing slot is a per-screen dispatcher (`trailingAction()` in
  `MobileWorkbench`): chat carries the Results entry, Results carries the way
...
folder. The pending kind lives in `MobileWorkbench` because the entry point
...
`MobileResultsView` is a workbench-scoped full-screen auxiliary surface. It
...
- Results do not depend on the current thread; they are workbench-scoped.
...
- Preserve the desktop/phone sibling-shell boundary. Do not add phone branches
  to `WorkbenchShell` or desktop slot layout.
