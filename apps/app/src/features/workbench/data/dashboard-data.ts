// @ts-nocheck
/**
 * dashboard-data — derives the workbench workspace's dashboard datasets from a
 ...
 * derived from the same fetch. Exposes `useWorkbenchThreadGroups` (grouping only,
 * for the thread list / chat dock) and `useWorkbenchDashboard` (full set, for
 * workbench home) so the grouping pass is shared rather than recomputed per view.
 *
...
import { useWorkbenchThreads } from "@/client/query/useWorkbenchThreads";
import { useWorks } from "@/client/query/useWorks";
import { useWorkbenchStore } from "@/client/stores";
...
export type WorkbenchThreadGroups = {
  workItems: WorkItem[];
...
export type WorkbenchDashboard = WorkbenchThreadGroups & {
...
/**
 * Group a workbench's real threads under their real work items.
 *
 * Primary threads are grouped by `thread.workId` against the workbench's works.
 ...
 *
 * This is the single source of grouping for the workbench dashboard — checkpoints
 * and activity are derived from the same fetch by {@link useWorkbenchDashboard},
...
export function useWorkbenchThreadGroups(workbenchId: string): WorkbenchThreadGroups {
  const { threads: realThreads } = useWorkbenchThreads(workbenchId);
  const { works } = useWorks(workbenchId);
...
/* ── Workbench dashboard (deep hook) ─────────────────────────────────── */

/**
 * The full workbench-home dataset from a single threads + works read: thread
 * groups, checkpoints, and the activity feed. Built on top of
 * {@link useWorkbenchThreadGroups} so the grouping pass is shared rather than
 * recomputed per derived view.
 *
 * Use this from the home dashboard. Views that only need the grouping (the
 * thread list, the chat dock) should call {@link useWorkbenchThreadGroups}
 * directly so they don't re-render on the activity clock tick.
 */
export function useWorkbenchDashboard(workbenchId: string): WorkbenchDashboard {
  const groups = useWorkbenchThreadGroups(workbenchId);
