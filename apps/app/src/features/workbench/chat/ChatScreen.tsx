// @ts-nocheck
/**
 * ChatScreen — workbench workspace destination that renders the selected thread
 ...
 *   3. Server `listWorkbenchThreads` fallback.
 */
export function ChatScreen({
  ...
  const pendingThreadId = useThreadStore((state) => {
    for (const [tid, ps] of Object.entries(state.pendingStreamByThreadId)) {
      if (ps.deferredSend?.workbenchId === workbenchId) return tid;
    }
    return null;
  });

  const { threads: projectThreads, isError, refetch } = useWorkbenchThreads(workbenchId);
    ...
    if (projectThreads !== null && projectThreads.length === 0) {
      return (
        <div className="grid h-full place-items-center px-6 text-sm text-muted-foreground">
          <Trans>This workbench has no conversations yet.</Trans>
        </div>
      );
    }
    return null;
...
function ChatScreenLoaded({
  ...
  const { threads: projectThreads } = useWorkbenchThreads(workbenchId);
