...
describe("resolveChatThreadId", () => {
  it("prefers URL selection over local fallbacks", () => {
    expect(
      resolveChatThreadId({
        ...
        projectThreads: [thread("workbench-thread")],
      }),
    ).toBe("url-thread");
  });

  it("uses the pending optimistic thread before fetched workbench threads", () => {
    expect(
      resolveChatThreadId({
  ...
  it("falls back to the first primary workbench thread", () => {
    expect(
      resolveChatThreadId({
        explicitThreadId: null,
