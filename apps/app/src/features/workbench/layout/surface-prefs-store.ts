// @ts-nocheck
/**
 * Device-local workbench surface preference store.
 *
...
export type SlotWidthBounds = Partial<Record<DesktopWorkbenchSlotId, { min: number; max: number }>>;
...
type PersistedSlotPrefsMap = Partial<Record<DesktopWorkbenchSlotId, Partial<SurfacePrefs>>>;
...
export function normalizeSurfacePrefs(prefs?: PersistedSurfacePrefsMap | null): SurfacePrefsMap {
  ...
  for (const id of WORKBENCH_SURFACE_IDS) {
 ...
 * ignores it (only writes ids in WORKBENCH_SURFACE_IDS) so it falls away.
 */
...
export const useWorkbenchSurfacePrefsStore: SurfacePrefsStore = create<SurfacePrefsSlice>()(
  devtools(
    persist(
      (set) => ({
      ...
      {
        name: "meridian:workbench-surface-layout",
        version: 3,
    ...
    { name: "workbench-surface-layout", enabled: import.meta.env.DEV },
  ),
);

/** Stable selector for surface preference actions (avoids re-renders on state changes). */
export function useWorkbenchSurfacePrefsActions(): SurfacePrefsActions {
  return useWorkbenchSurfacePrefsStore(
    useShallow((state) => ({
