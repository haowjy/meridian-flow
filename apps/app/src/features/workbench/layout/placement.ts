// @ts-nocheck
/**
 * Pure screen-to-surface placement for the workbench workbench.
 *
...
function assertOneActiveSurfacePerSlot(layout: SurfaceLayoutMap): void {
  const surfaceBySlot: Partial<Record<DesktopWorkbenchSlotId, SurfaceId>> = {};
  for (const surfaceId of Object.keys(layout) as SurfaceId[]) {
    ...
    if (existingSurfaceId) {
      throw new Error(
        `Workbench layout invariant failed: slot "${slot}" has multiple active surfaces (${existingSurfaceId}, ${surfaceId}).`,
      );
    }
...
export function useWorkbenchLayout(screen: ScreenKey): SurfaceLayoutMap {
  const prefs = useWorkbenchSurfacePrefsStore((state) => state.prefs);
  const slotPrefs = useWorkbenchSurfacePrefsStore((state) => state.slotPrefs);
  return useMemo(
