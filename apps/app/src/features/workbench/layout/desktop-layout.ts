// @ts-nocheck
/**
 * Desktop workbench slot topology and grid structure for stable surfaces.
 *
 * Purpose: define the real workbench workbench slots as data instead of baking
 ...
 */
import type { DesktopWorkbenchSlotId, SlotDefinition } from "./types";

export type { DesktopWorkbenchSlotId } from "./types";

export const DESKTOP_WORKBENCH_SLOTS: SlotDefinition[] = [
...
export function getDesktopGridTemplate(): {
  ...
  return {
    ...
    columns:
      "var(--workbench-left-width) var(--workbench-left-handle-width) minmax(var(--workbench-main-min-width), 1fr) var(--workbench-dock-handle-width) var(--workbench-dock-width)",
  };
}

export type DesktopWorkbenchSlots = readonly DesktopWorkbenchSlotId[];
