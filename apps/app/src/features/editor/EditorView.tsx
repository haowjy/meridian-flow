...
export function EditorView({
  ...
  const handleFigureFile = useCallback(
    async (file: File, insertPos?: number) => {
      if (!workbenchId) {
        setFigureUploadState({
          kind: "error",
          message: t`A workbench is required before figures can be uploaded.`,
        });
        return;
      }
    ...
    [clearUploadLater, documentId, workbenchId],
  );
  ...
  const editor = useEditor(
    {
      ...(session
        ? createEditorConfig({
            ...
            editable,
            autofocus: false,
            figureRenderContext: { workbenchId, documentId },
      ...
      {showToolbar ? (
        <div className="flex shrink-0 items-center border-b border-border bg-background px-2 py-1.5">
          <EditorToolbar
            ...
            figureUploadDisabled={!workbenchId}
