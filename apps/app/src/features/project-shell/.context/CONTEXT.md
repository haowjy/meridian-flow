# features/project — Desktop project workspace shell

The authenticated project project workspace: one persistent multi-panel desktop surface
...
Design intent lives in [`DESIGN.md` § Project workspace shell](../../../../../../DESIGN.md);
the model/view continuity rationale is the KB decision
[persistent-surfaces-lift](https://github.com/meridian-bio/docs/blob/main/kb/decisions/persistent-surfaces-lift.md).
This page is the *implementation* contract.

Mobile now lives beside this desktop shell as a sibling implementation. The
shared `Project workspaceView` selects `mobile/MobileProject workspace` only through the phone
capability predicate in `usePhoneShell()`; desktop `Project workspaceShell` remains the
...
`layout/SlotGrid.tsx` renders the whole project workspace as a **single flat CSS grid**.
...
in the same `_authenticated.tsx` effect behind the same project workspace gate.

`layout/surface-prefs-store.ts` is the **device-local** chrome-prefs store
(Zustand `persist`, localStorage key `meridian:project workspace-surface-layout`,
...
2. `routes/_authenticated.tsx` calls `useProject workspaceSurfacePrefsStore.persist
   .rehydrate()` (synchronous — localStorage) then `setHydrated()` immediately.
3. **`ProjectView` gates the whole `DesktopProject workspace` mount on `_hydrated`** so it
   mounts exactly once against final persisted prefs (at most one frame, no
   visible flash).

Two rules keep this stable:

- **Gate at the parent (`ProjectView`), never inside `DesktopProject workspace`.** A
...
- **`DesktopProject workspace` takes a single merged layout subscription**
  (`useProject workspaceLayout`). Do not add a second whole-prefs subscription — that
