// @ts-nocheck
/**
 * ConversationSection — which thread the workbench currently treats as active,
 ...
 *    (inspect the `["workbenches", workbenchId, "threads"]` query).
 *  - raw WS frames → Chrome DevTools → Network → WS → Messages.
 *
 * Resolution order: `streamingThreadId` (primary) → route (`/chat/$threadId`
 * path param, or the workbench route's `?thread=…` search param — note `thread`,
 * not `threadId`) → none.
 *
