 ...
 *
 * Master-detail Library over `WorkbenchLibraryResponse`: grouped inventory on
...
import { useWorkbenchLibrary } from "@/client/query/useWorkbenchLibrary";
import { Skeleton } from "@/components/ui/skeleton";
import { AgentChip } from "@/features/agents";
import { SidebarSectionLabel } from "@/features/workbench/shell/SidebarSectionLabel";
...
export function LibraryScreen({ workbenchId, onTestAgent }: LibraryScreenProps) {
  const { library, status, isError, refetch } = useWorkbenchLibrary(workbenchId);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
      <p className="text-sm text-muted-foreground">
        <Trans>Could not load the library for this workbench.</Trans>
      </p>
      <button
        type="button"
        onClick={onRetry}
