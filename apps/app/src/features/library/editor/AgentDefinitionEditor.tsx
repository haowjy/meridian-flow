...
export function AgentDefinitionEditor({
  ...
  const { data, isPending, isError, refetch } = useAgentDefinition(workbenchId, slug);
  const updateMutation = useUpdateAgentDefinition(workbenchId, slug);
  const patchSkillLink = usePatchAgentSkillLink(workbenchId, slug);
  const restoreRevision = useRestoreAgentDefinitionRevision(workbenchId, slug);
  const restoreOriginal = useRestoreAgentDefinitionOriginal(workbenchId, slug);
