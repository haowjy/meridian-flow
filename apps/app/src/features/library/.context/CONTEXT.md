# features/library — Workbench capability inventory and authoring surface

The Library is the workbench's first-class capability screen. It lists the
agents, skills, and installed packages a workbench owns, and provides the
...
The Library uses `GET /api/workbenches/:workbenchId/library`. That inventory is
...
```mermaid
flowchart TD
  Route[Workbench screen=library] --> Library[LibraryScreen]
...
| `LibraryPaneController.tsx` | Connects the screen to workbench routing and dock thread selection. |
...
| `client/api/workbench-definitions-api.ts` | Definition CRUD/revision HTTP client. |
