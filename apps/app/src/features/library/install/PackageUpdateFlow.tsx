...
import { workbenchPackageExportPath } from "@/client/api/package-install-api";
...
export type PackageDetailPanelProps = {
  ...
  library: WorkbenchLibraryResponse;
  className?: string;
  onCloseUpdate?: () => void;
};
...
export function PackageDetailPanel({
  ...
  function handleExport() {
    window.location.assign(workbenchPackageExportPath(workbenchId, pkg.installId));
  }
...
export function PackageUpdateFlow({
  ...
  const checkQuery = usePackageUpdateCheck(workbenchId, installId, true);
  const applyMutation = useApplyPackageUpdate(workbenchId, installId);
...
function UpdateItemRestoreOriginal({
  ...
  const restoreAgent = useRestoreAgentDefinitionOriginal(workbenchId, slug);
  const restoreSkill = useRestoreSkillDefinitionOriginal(workbenchId, slug);
...
function buildNameBySlug(library: WorkbenchLibraryResponse): Map<string, string> {
