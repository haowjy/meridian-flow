...
export function PackageInstallFlow({ projectId, onCancel }: PackageInstallFlowProps) {
  const catalogQuery = usePackagesCatalog();
  const previewMutation = usePreviewPackageInstall(projectId);
  const applyMutation = useApplyPackageInstall(projectId);
