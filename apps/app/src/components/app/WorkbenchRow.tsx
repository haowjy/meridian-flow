// @ts-nocheck
/**
 * WorkbenchRow — editable workbench row for the desktop app sidebar.
 *
...
import { announce, useWorkbenchActions } from "@/client/stores";
...
export function WorkbenchRow({
  ...
  const title = displayWorkbenchTitle(workbench.title);
  ...
  const commit = useCallback(() => {
    ...
    if (trimmed) {
      actions.rename(workbench.id, trimmed);
    }
    setMode("view");
    const resolved = trimmed || displayWorkbenchTitle(null);
    announce(t`Renamed to ${resolved}`);
  }, [actions, draft, workbench.id]);
  ...
  function handleTrashClick(event: MouseEvent<HTMLButtonElement>) {
    event.preventDefault();
    event.stopPropagation();
    if (!actions.softDelete(workbench.id, workbench)) return;
    if (isActive) {
      void navigate({ to: "/" });
    }
    announce(t`Workbench deleted. Press undo to restore.`);
  }
