...
function useFigureRenderState(input: {
  ...
  const { workbenchId, documentId, src } = input;
  ...
  useEffect(() => {
    ...
    if (!workbenchId || !documentId) {
      setState({
        kind: "error",
        url: null,
        message: t`This stored figure needs a workbench and document before it can be rendered.`,
      });
      return;
    }
    ...
    const routeWorkbenchId = workbenchId;
  ...
  }, [documentId, workbenchId, refreshToken, src]);
...
export function FigureNodeView(props: NodeViewProps) {
  ...
  const { workbenchId, documentId } = getExtensionOptions(props);
