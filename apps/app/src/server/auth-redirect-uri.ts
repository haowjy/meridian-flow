...
/**
 * Resolve the auth callback URI for providers that need one.
 *
 * Supabase dev-login does not use this path, but keeping the seam lets OAuth or
 * magic-link flows share the same portless/Tailscale-safe redirect behavior.
 */
export function resolveAuthRedirectUri(request: Request): string {
  const { runtime, supabaseAuthRedirectUri: fallback } = getAppServerConfig();
  if (!fallback) {
    throw new Error("SUPABASE_AUTH_REDIRECT_URI is required for callback-based auth flows.");
  }
