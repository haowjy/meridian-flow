 ...
 *
 * Wraps the workbench/document figure endpoints, seeds and reads a signed-URL
...
function figurePath({ workbenchId, documentId }: FigureRouteInput): string {
  return `/api/workbenches/${encodeURIComponent(workbenchId)}/documents/${encodeURIComponent(
    documentId,
  )}/figure`;
}
...
function cacheKey(input: FigureRouteInput & { storageUrl: string }): string {
  return `${input.workbenchId}\u0000${input.documentId}\u0000${input.storageUrl}`;
}
