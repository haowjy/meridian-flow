 ...
 *
 * Phase 1: workbench rows live in React Query; workbench-level soft-delete lives
 * in `WorkbenchStoreProvider`. This store now holds only per-thread turns,
 * handoff flags, streaming coordination, and the pending-creation gate that
 * the optimistic Home → Workbench flow uses to suppress fetches until the
 * server-side workbench + thread exist. Live assistant blocks are written into
...
import { workbenchQueryKeys } from "@/client/query/workbench-query-keys";
import {
  patchThreadInProjectCaches,
  upsertThreadInProject,
} from "@/client/query/workbench-thread-cache";
import {
...
/**
 * Pending-creation gate. While a workbench / thread is being created on the
...
type PendingCreationState = {
  workbenchIds: Record<string, true>;
...
function liveThreadListPatchForTurnStatus(
  ...
  if (status === "waiting_checkpoint") {
    // A pending checkpoint is an in-run pause for human input. The workbench
  ...
  return createStore<ThreadStoreSlice>()(
    devtools(
      (set, get) => ({
        ...
        pendingCreation: { workbenchIds: {}, threadIds: {} },
        streamingThreadId: null,
        streamingWorkbenchId: null,
        turnCounter: 0,
        ...
        setStreamingThreadId(id, workbenchId = null) {
          set({ streamingThreadId: id, streamingWorkbenchId: id ? workbenchId : null });
        },
        ...
        patchTurnStatus(threadId, turnId, status, patch = {}) {
          let terminalWorkbenchId: string | null = null;
          ...
          set((state) => {
            ...
            terminalWorkbenchId =
              state.streamingThreadId === threadId ? state.streamingWorkbenchId : null;
          ...
          queueMicrotask(() => {
            ...
            if (terminalWorkbenchId) {
              void queryClient.invalidateQueries({
                queryKey: workbenchQueryKeys.threads(terminalWorkbenchId),
              });
            }
          });
        },
        ...
        applyThreadSnapshot(thread, serverTurns, lifecycle) {
          ...
          /**
           * Handoff: the optimistic Home → Workbench navigation flow.
           *
           * When the user creates a workbench from Home, the client
           * optimistically creates the thread + workbench and navigates
        ...
        clearPendingCreation({ workbenchId, threadId }) {
          set((state) => {
            const workbenchIds = { ...state.pendingCreation.workbenchIds };
            const threadIds = { ...state.pendingCreation.threadIds };
            if (workbenchId) delete workbenchIds[workbenchId];
            if (threadId) delete threadIds[threadId];
            return { pendingCreation: { workbenchIds, threadIds } };
          });
        },
      }),
...
/** True when an optimistic workbench create is in flight (pre-server confirmation). */
export function useIsWorkbenchPendingCreation(workbenchId: string | null | undefined): boolean {
  return useThreadStore((s) =>
    workbenchId ? Boolean(s.pendingCreation.workbenchIds[workbenchId]) : false,
  );
}
