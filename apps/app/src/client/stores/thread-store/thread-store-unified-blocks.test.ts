 ...
 * store mutations plus their workbench-thread-list lifecycle cache patches.
 *
...
import { lifecycleFor } from "@/features/workbench/lifecycle";
...
function cachedThreadListItem(queryClient: QueryClient): ThreadListItem {
  const item = queryClient.getQueryData<ThreadListItem[]>(
    workbenchQueryKeys.threads(PROJECT_ID),
  )?.[0];
...
describe("unified-block store actions", () => {
  ...
  it("patches workbench thread-list lifecycle through checkpoint resolve", () => {
    const queryClient = new QueryClient();
    queryClient.setQueryData(workbenchQueryKeys.threads(PROJECT_ID), [threadListItem()]);
