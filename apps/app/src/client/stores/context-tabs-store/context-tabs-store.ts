// @ts-nocheck
/**
 * Context tabs store — per-workbench open-file working set for the Context
 * destination.
 *
...
type ContextTabsState = {
  /** workbenchId → slice. One tab list per workbench. */
  byProject: Record<string, ProjectTabsSlice>;
};

type ContextTabsActions = {
  openTab: (workbenchId: string, tab: ContextTab) => void;
  /**
   ...
   */
  closeTab: (workbenchId: string, documentId: string) => ContextTab | null;
  reorderTabs: (workbenchId: string, fromIndex: number, toIndex: number) => void;
  /** Clear every tab for a workbench — used when the workbench is deleted. */
  clearProject: (workbenchId: string) => void;
};
...
function sliceFor(state: ContextTabsState, workbenchId: string): ProjectTabsSlice {
  return state.byProject[workbenchId] ?? emptySlice();
}
...
function patchSlice(
  ...
  return { ...state, byProject: { ...state.byProject, [workbenchId]: next } };
}
  ...
  devtools(
    (set, get) => ({
      ...
      openTab: (workbenchId, tab) => {
        set((state) => {
          const slice = sliceFor(state, workbenchId);
          ...
          const nextTabs = exists
            ...
            : [...slice.tabs, tab];
          return patchSlice(state, workbenchId, { tabs: nextTabs });
        });
      },

      closeTab: (workbenchId, documentId) => {
        const slice = sliceFor(get(), workbenchId);
        ...
        set((state) => patchSlice(state, workbenchId, { tabs: nextTabs }));
        return fallback;
      },

      reorderTabs: (workbenchId, fromIndex, toIndex) => {
        set((state) => {
          const slice = sliceFor(state, workbenchId);
          if (
            fromIndex === toIndex ||
          ...
          return patchSlice(state, workbenchId, { tabs: next });
        });
      },

      clearProject: (workbenchId) => {
        set((state) => {
          if (!state.byProject[workbenchId]) return state;
          const { [workbenchId]: _removed, ...rest } = state.byProject;
...
/** Selector helper — returns the tab slice for a workbench (stable empty default). */
export function useContextTabs(workbenchId: string): ProjectTabsSlice {
  return useContextTabsStore(useShallow((s) => s.byProject[workbenchId] ?? EMPTY_SLICE));
}
