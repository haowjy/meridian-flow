...
|------|----------|----------|
| Foreground (default) | Tool blocks on channel/waitgroup. Child completion sends result, tool returns `tool_result` normally. Enforces `spawn_timeout` (default 5 minutes) via `context.WithTimeout`. On timeout, child is cancelled and tool returns timeout error. | Slow tool call |
| Background (post-v1) | Returns immediately with task handle. Real work in goroutine. See [background-execution](background-execution.md). Deferred to post-v1 — foreground-only for v1. | `{"task_id": "bg_abc123", "status": "running"}` |

## Spawn Limits

- **Max spawn depth**: 3 (configurable). Denormalized as `spawn_depth INTEGER DEFAULT 0` on thread row — child sets `spawn_depth = parent.spawn_depth + 1`. Depth check is O(1), no chain walk. DB constraint prevents `parent_thread_id = id`.
- **Max concurrent spawns per work item**: 5 (configurable).
- **Spawn timeout**: 5 minutes (foreground, configurable). Enforced via `context.WithTimeout` wrapping the child streaming context.
