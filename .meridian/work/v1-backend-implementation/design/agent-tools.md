...
All write surfaces MUST enforce this exact order:

1. **Canonicalize** with `filepath.Clean` — resolves `..`, `.`, double slashes
2. **Namespace detection** — determine which namespace the clean path belongs to
3. **Work-item isolation** — if `.meridian/work/`, enforce `workItemSlug` prefix match

Never prefix-match on uncanonicalized paths. Example attack: `/.meridian/work/my-slug/../../.agents/evil.md` → `filepath.Clean` → `/.agents/evil.md` → namespace detection correctly identifies `.agents/` namespace → review-gated, not work-item path.

All write surfaces — TextEditor, bash virtual FS, any future write tool — MUST use a shared isolation function, not per-tool duplication.
