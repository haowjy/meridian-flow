# Activity Stream — Per-Item Collapse Defaults

Redesigned ActivityBlock for the biomedical MVP. One block per assistant turn. Each item has a per-item collapse default based on its kind and tool category. Per-tool-category display config determines collapse behavior. See [overview](overview.md) for frontend architecture context.
...
**Key principle**: Items that are never collapsed contain everything the researcher needs — text, images, charts, tables, mesh cards, and stdout from tools that default to uncollapsed output. Items that are collapsed by default contain implementation details (thinking, tool inputs, stdout from tools that collapse by default). Results are inline content, interleaved with text. User can toggle any item.
...
export const TOOL_DISPLAY_CONFIG: Record<string, ToolDisplayConfig> = {
  python: {
    input: "collapsed",     // Code collapsed by default
    stdout: "visible",      // Output uncollapsed by default
    stderr: "hidden",       // Click for popup
  },
  bash: {
    input: "collapsed",     // Command collapsed by default
    stdout: "collapsed",    // Output collapsed by default
    stderr: "hidden",       // Click for popup
  },
  read: {
    input: "collapsed",
...
/**
 * Rich display result. Renders inline in the ActivityBlock, never collapsed by default.
...
## ActivityBlock Rendering — Per-Item Collapse Defaults

### Item Separation

ActivityBlock separates items into collapsed and uncollapsed groups based on kind and tool display config:

```typescript
function useCollapseDefaults(items: ActivityItem[]) {
  return useMemo(() => {
    ...
    for (const item of items) {
      switch (item.kind) {
        ...
        case "tool":
          // Tool row collapsed by default (shows input, status)
          collapsed.push(item)
          // But stdout may be uncollapsed based on tool config
...
### Uncollapsed Tool stdout

For tools whose display config says `stdout: "visible"` (like `python`), the stdout is uncollapsed by default even though the tool row itself is collapsed:
...
function ActivityBlock({ activity }: Props) {
  const { collapsed, visible } = useCollapseDefaults(activity.items)
  ...
  return (
    <div>
      ...
      {/* Uncollapsed items — always shown, interleaved content */}
      {visible.map(item => {
...
**Note**: The exact interleaving of text, results, and stdout among uncollapsed items follows insertion order from the items array. Content, display results, and uncollapsed stdout all appear in the order they were received.
...
Shows the Python code input when the tool row is expanded (collapsed by default):
...
- `display_result` blocks into `DisplayResultItem` entries (never collapsed by default)
...
export const PYTHON_ANALYSIS_SCENARIO: StreamEvent[] = [
  ...
  // Stdout streams uncollapsed (python stdout is uncollapsed by default)
