...
export function CompactBreadcrumb({
  ...
  const singleSegmentClass =
    singleSegmentVariant === "nonLast" ? nonLastTextClass : lastTextClass;
          ...
          <Fragment key={index}>
            ...
            {segment.onClick && !isLast ? (
              <button
                ...
                className={cn(
                  "cursor-pointer truncate hover:underline focus:outline-none focus-visible:underline",
                  segmentTone,
                  segment.className,
                )}
                title={segment.title}
