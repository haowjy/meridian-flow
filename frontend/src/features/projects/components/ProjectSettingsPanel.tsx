...
function SectionHeader({
  ...
  return (
    <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-lg md:text-sm font-medium text-foreground hover:bg-[var(--hover)] transition-colors rounded-sm">
      <span className="flex items-center gap-2">
        {title}
        {badge !== undefined && (
          <span className="text-sm md:text-xs text-muted-foreground">({badge})</span>
        )}
      </span>
      <span className="flex items-center gap-2">
        ...
        <ChevronDown
          className={cn(
            'size-4 md:size-4.5 text-muted-foreground transition-transform duration-200',
            open && 'rotate-180'
          )}
        />
      </span>
...
function SettingsRow({
  ...
  return (
    <div
      className={cn(
        'flex min-h-10 md:min-h-7 items-center justify-between gap-3 py-0.5 px-3',
        indent && 'pl-6',
        className
      )}
    >
...
function SettingsGroupHeader({
  ...
  return (
    <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-1 text-base md:text-xs font-medium text-muted-foreground uppercase tracking-wide hover:bg-[var(--hover)] transition-colors rounded-sm">
      <span className="truncate">{title}</span>
      <ChevronDown
        className={cn(
          'size-5 md:size-3.5 text-muted-foreground transition-transform duration-200',
          open && 'rotate-180'
        )}
      />
    </CollapsibleTrigger>
      ...
      right={
        <div className="flex items-center gap-2">
          <Button
            ...
            size="icon-sm"
            onClick={onDelete}
            className="size-8 md:size-6.5 opacity-60 md:opacity-0 md:group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-error"
          >
 ...
 */
export function ProjectSettingsPanelContent({ projectId }: ProjectSettingsPanelContentProps) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto pt-2">
      {/* Error display */}
      {error && (
        <div className="mx-3 mb-3 p-2 text-sm text-error bg-error/10 rounded-sm flex items-center justify-between">
          <span>{error}</span>
          <Button
            variant="ghost"
            size="icon-xs"
          ...
          actions={
            isInstructionsDirty && (
              <>
                ...
                >
                  {isSavingInstructions && <Loader2 className="mr-1 size-2.5 animate-spin" />}
                  Save
                </Button>
              </>
            )
