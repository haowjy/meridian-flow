...
import type { BulkOperation, OperationContext, OperationResult } from '../operations/types'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function BatchActionsBar({ operations, selectedItems, context, onComplete }: BatchActionsBarProps) {
  ...
  return (
    <>
      ...
      {confirmOperation && (
        <Dialog open={!!confirmOperation} onOpenChange={(open) => !open && setConfirmOperation(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
