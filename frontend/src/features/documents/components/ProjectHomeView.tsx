 ...
 */
export function ProjectHomeView({ projectId, projectSlug }: ProjectHomeViewProps) {
  return (
...
function RecentDocumentsSection({ projectId, projectSlug }: ProjectHomeViewProps) {
        ...
        {resolvedDocs.map((doc) => {
          ...
          return (
            <button
              key={doc.id}
              type="button"
              onClick={() => openDocument(doc.id, doc.path, projectSlug, navigate)}
              className="group flex w-full items-center gap-2 rounded-sm px-1.5 py-1 text-left text-sm transition-colors hover:bg-hover"
            >
