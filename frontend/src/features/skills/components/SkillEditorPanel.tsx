...
export function SkillEditorPanel({
  ...
  const [descriptionApiError, setDescriptionApiError] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function loadSkill() {
      ...
      try {
        const loadedSkill = await api.skills.get(projectId, skillId, {
  ...
  const descriptionError =
    touchedFields.has("description") && isDescriptionEmpty
      ? "Description is required"
      : undefined;
  const contentError =
    touchedFields.has("content") && isContentEmpty
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (localDescription.trim() !== skill.description)
        updates.description = localDescription.trim();
      if (localContent.trim() !== skill.content)
        updates.content = localContent.trim();
  ...
  return (
    <div className="bg-background flex h-full flex-col overflow-hidden">
      <SkillForm
        ...
        headerTitle={<span className="text-sm font-medium">/{localName}</span>}
        headerTrailing={
