/**
 * Inline Diff Preview for edit commands (str_replace, insert, append, create).
 *
 ...
 *
 * Moved from DocEditBlock/ — used by TextEditorBlock for edit command rendering.
 */
...
/** Input fields relevant to diff preview (edit commands only) */
export interface DiffPreviewInput {
  command: "str_replace" | "insert" | "append" | "create";
  path: string;
  old_str?: string;
  new_str?: string;
  insert_line?: number;
  ...
  if (!meta.previewHead && !meta.previewTail) {
    return (
      <div className="text-muted-foreground py-2 text-xs italic">
        Content too large to preview ({meta.charCount?.toLocaleString() ?? "?"}{" "}
        chars)
  ...
  return (
    <pre
      className={cn(
        ...
        "bg-success/10 border-success/50 border-l-2",
    ...
    >
      ...
      {meta.previewTail && meta.previewTail !== meta.previewHead && (
        <>
          <span className="text-muted-foreground/70 my-1.5 block text-center text-[10px] select-none">
...
export const DiffPreview = React.memo(function DiffPreview({
  input,
  isStreaming = false,
  truncationMeta,
}: DiffPreviewProps) {
        ...
        {diffs.map(([op, text], i) => (
          <span
            ...
            className={cn(
              ...
              op === -1 &&
                "bg-error/20 text-error decoration-error/60 line-through",
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center gap-2 text-[11px]">
        <span className="text-success font-medium">+ Insert</span>
...
function AppendPreview({
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center gap-2 text-[11px]">
        <span className="text-success font-medium">+ Append</span>
        <span className="text-muted-foreground">
          to end of document
          {(hasContent || isTruncated) && (
            <span className="ml-1">
...
function CreatePreview({
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center gap-2 text-[11px]">
        <span className="text-success font-medium">+ Create</span>
        <span className="text-muted-foreground">
          new file
          {(hasContent || isTruncated) && (
            <span className="ml-1">
