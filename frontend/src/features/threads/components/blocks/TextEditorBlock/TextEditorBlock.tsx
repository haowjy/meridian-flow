...
function getViewResult(
  toolResult: TurnBlock | null,
): TextEditorDocumentResult | TextEditorFolderResult | null {
...
function DocumentPreview({ result }: { result: TextEditorDocumentResult }) {
  ...
  const parsed = useMemo(
    () => parseLineNumberedContent(displayContent),
    [displayContent],
  );
...
export const TextEditorBlock = React.memo(function TextEditorBlock({
  ...
  const path =
    (streamingInput?.path as string | undefined) ?? fallbackInput?.path;
  const command =
    (streamingInput?.command as TextEditorCommand | undefined) ??
    fallbackInput?.command;
  const viewRange =
    (streamingInput?.view_range as [number, number] | undefined) ??
    fallbackInput?.view_range;
  const oldStr =
    (streamingInput?.old_str as string | undefined) ?? fallbackInput?.old_str;
  const newStr =
    (streamingInput?.new_str as string | undefined) ?? fallbackInput?.new_str;
  const fileText =
    (streamingInput?.file_text as string | undefined) ??
    fallbackInput?.file_text;
  const insertLine =
    (streamingInput?.insert_line as number | undefined) ??
  ...
  const Icon =
    input && isViewCommand(input.command)
      ? isFolderRes
        ? FolderOpen
        : FileText
      : FileEdit;
  ...
  const documentNoLongerExists =
    isDocResult && viewResult.id && !resolvedDocument;
  ...
  return (
    ...
    >
      ...
      {!hasResult &&
        ...
        toolState === null &&
        input &&
        isViewCommand(input.command) && <div className="py-2" />}
    </CollapsibleToolBlock>
  );
});
