...
function getToolMeta(
  ...
  const toolUseId =
    typeof toolUseIdRaw === "string"
      ? normalizeToolCallId(toolUseIdRaw)
      : toolUseIdRaw;
  const isError =
    typeof content.isError === "boolean" ? content.isError : false;
...
export const ToolInteractionBlock = React.memo(function ToolInteractionBlock({
  ...
  const title = isPending
    ? "Pending..."
    : (callPreview ?? (shortId ? `${displayName} (${shortId})` : displayName));
  ...
  const isGenerating =
    !hasResult &&
    !isError &&
    (toolState === null || toolState === ToolStreamState.PREPARING);
  ...
  return (
    ...
    >
      ...
      {!hasResult &&
        !isError &&
        toolState === ToolStreamState.PREPARING &&
        activeArgKey && (
          <div className="bg-muted/20 rounded-md border px-2.5 py-2 text-xs">
              ...
              <div className="min-w-0 truncate">
                ...
                {typeof activeArgChars === "number" ? (
                  <span className="text-muted-foreground/80">
                    {" "}
                    ({activeArgChars.toLocaleString()} chars)
                  </span>
                ) : null}
              </div>
              <div className="text-muted-foreground/80 shrink-0">
                {typeof argsTotalBytes === "number"
                  ? formatBytes(argsTotalBytes)
                  : null}
              </div>
            </div>
            {(activeArgPreviewHead || activeArgPreviewTail) && (
              <pre className="bg-muted/30 mt-2 max-h-24 overflow-auto rounded-md px-2 py-1.5 text-xs whitespace-pre-wrap">
                {activeArgPreviewHead}
                {activeArgPreviewTail &&
                activeArgPreviewTail !== activeArgPreviewHead ? (
                  <>
                    {"\n…\n"}
                    {activeArgPreviewTail}
                  </>
      ...
      {toolUse && (toolUse.content as ToolBlockContent)?.input && (
        <div>
          <div className="text-muted-foreground/80 mb-1 text-xs font-medium">
            Call
          </div>
          <pre className="bg-muted/30 max-h-40 overflow-auto rounded-md border px-2.5 py-2 text-xs whitespace-pre-wrap">
            {safeJsonStringify(
              (toolUse.content as ToolBlockContent).input ?? {},
            )}
          </pre>
        </div>
      )}
