...
allowed-tools: Bash(*/orchestrate/scripts/run-agent.sh *), Bash(*/orchestrate/scripts/save-handoff.sh *), Bash(git *), Bash(cat *), Bash(mkdir *), Bash(rm */.runs/*), Bash(cp *), Bash(ln *), Bash(date *)
---

# Orchestrate — Supervisor Skill

> **ROLE: You are a SUPERVISOR, not an implementer.**
> You compose context and launch agents via `scripts/run-agent.sh`.
...
3. **Every slice MUST go through `scripts/run-agent.sh`.** No exceptions, even if trivial.
...
Use `scripts/run-agent.sh` for everything. Log directories are auto-derived from scope variables (`SLICE_FILE`, `SLICES_DIR`, etc.) — you don't need to set `ORCHESTRATE_LOG_DIR` unless overriding for parallel runs.
...
scripts/run-agent.sh implement -v SLICE_FILE=$SLICE_ROOT/slice.md

# Pass reference files (appended as a "Reference Files" section in prompt)
scripts/run-agent.sh implement \
...
ORCHESTRATE_LOG_DIR=$SLICE_ROOT/logs/agent-runs/review-opus \
  scripts/run-agent.sh review -m claude-opus-4-6 &
wait
```
...
```bash
scripts/run-agent.sh plan-slice \
