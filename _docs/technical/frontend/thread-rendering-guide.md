...
**See also:**
- `thread-rendering.md` - Decision rationale and quick reference
- `architecture/thread-rendering-research.md` - Full research and alternatives
...
flowchart TB
    ...
    Handler -->|update| Store[useThreadStore]
...
```typescript
// frontend/src/features/threads/components/AssistantTurn.tsx
...
```typescript
// frontend/src/features/threads/hooks/useStreamingBuffer.ts
...
```typescript
// frontend/src/features/threads/hooks/useThreadSSE.ts
export function useThreadSSE(threadId: string) {
  const { append, flush } = useStreamingBuffer({
    flushInterval: 50,
    onFlush: (content) => {
      useThreadStore.getState().updateActiveBlock(content);
    }
  });
...
```typescript
// frontend/src/features/threads/components/CodeBlock.tsx
import { Streamdown } from 'streamdown';
...
```typescript
// frontend/src/features/threads/components/ThinkingBlock.tsx
...
```typescript
// frontend/src/features/threads/components/MessageList.tsx
...
```typescript
// frontend/src/features/threads/components/ChatHistory.tsx
...
Current: `frontend/src/features/threads/components/ActiveThreadView.tsx`
...
- SSE events: `backend/internal/handler/sse_handler.go`
- Current thread UI: `frontend/src/features/threads/components/ActiveThreadView.tsx`
- Thread store: `frontend/src/core/stores/useThreadStore.ts`
