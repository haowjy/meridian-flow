...
- Every conversation gets a short server-assigned handle from the project's one counter: `c1`, `c2` for primary chats, `s1`, `s2` for subagents. The model addresses a child by its handle (`continue` takes `handle`), so the `spawn`/`continue` tool traffic it re-reads every turn carries `s3` instead of a 36-character UUID. The handle is not the id: the chat URL, local store key, and Open door stay on the UUID, and the writer only sees `cN` as a copyable handle.
...
- `ArtifactCard` is the shared shell for `ask_user`, `spawn`, and `child-report` artifact cards, with a door slot, running/failed tones, and an optional title. `SpawnReportCard` is a thin adapter over it and drops its separate muted strip.
