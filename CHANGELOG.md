...
### Added
- Added WS-D onboarding with persistent Setup-agent project creation, `/onboarding` guided flow, import-or-chat path, and authenticated redirect gate.
...
### Fixed
- **v3 launch:** return Setup, Muse, Spark, Writer, and none from Home agent catalog before workbench seeding.
- **server/billing:** enforce subscription credit webhook idempotency with a DB-unique subscription reason key.
- **v3 launch:** fix billing proxy ownership/scope, subscription grant rows, handoff/fork document inheritance, background return-result aborts, and launch-agent loading in Nitro.
