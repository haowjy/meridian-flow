...
type ThreadRepository interface {
	...
	CreateThread(ctx context.Context, thread *llm.Thread) error

	// GetThread retrieves a thread by ID (scoped to user)
	// Returns domain.ErrNotFound if not found
	GetThread(ctx context.Context, threadID, userID string) (*llm.Thread, error)
	...
	// Returns domain.ErrNotFound if not found
	GetThreadByIDOnly(ctx context.Context, threadID string) (*llm.Thread, error)

	// ListThreadsByProject retrieves all threads for a project
	// Returns empty slice if no threads found
	ListThreadsByProject(ctx context.Context, projectID, userID string) ([]llm.Thread, error)

	// UpdateThread updates a thread's mutable fields (title, last_viewed_turn_id, updated_at)
	// Returns domain.ErrNotFound if not found
	UpdateThread(ctx context.Context, thread *llm.Thread) error
	...
	DeleteThread(ctx context.Context, threadID, userID string) (*llm.Thread, error)
	...
	GetThreadTree(ctx context.Context, threadID, userID string) (*llm.ThreadTree, error)
}
