...
// It tries UUID first (most common), then slug, allowing extensibility for
// future identifier types (short codes, aliases, etc.)
...
func (r *ChainedResolver) ResolveDocument(ctx context.Context, identifier, projectID string) (string, error) {
	...
	// Strategy 2: Try as slug
	doc, err := r.documentRepo.GetBySlug(ctx, identifier, projectID)
	if err == nil {
		return doc.ID, nil
	}
	if !errors.Is(err, domain.ErrNotFound) {
...
// Only works for UUIDs - slugs require project context.
// Use ResolveDocument() when projectID is available for full slug support.
