...
- **The app-boot smoke owns its evidence.** It probes only the spawned child's reported origin while that child is alive. Keep the exact route contract in `lib/app-boot-contract.ts`: `/` is 307; `/login` is 200 with the Meridian login marker. A foreign listener or generic non-5xx response is not success.
