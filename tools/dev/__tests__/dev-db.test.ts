...
describe("dev-db", () => {
  ...
  it("recognizes local database hosts", () => {
    expect(isLocalDatabaseHost("postgresql://postgres:postgres@127.0.0.1:54422/meridian")).toBe(
      true,
    );
    expect(isLocalDatabaseHost("postgresql://postgres:postgres@localhost:54422/meridian")).toBe(
      true,
    );
    expect(isLocalDatabaseHost("postgresql://postgres:postgres@db.example.com:5432/meridian")).toBe(
      false,
    );
  });
